...
| com/adobe/granite/workflow/offloading | com.adobe.granite.workflow.core.offloading.WorkflowOffloadingJobConsumer | Procesa los trabajos que genera el flujo de trabajo del [!UICONTROL descargador] de recursos de actualización de DAM. |
...
De forma predeterminada, Experience Manager ejecuta el flujo de trabajo de recursos [!UICONTROL de actualización de] DAM cuando cambia un recurso de DAM o se agrega uno a DAM. Cambie el comportamiento predeterminado para que, en su lugar, Experience Manager ejecute el flujo de trabajo de descarga de recursos [!UICONTROL DAM Update] . Este flujo de trabajo genera un trabajo de JobManager que tiene un tema de `com/adobe/granite/workflow/offloading`. A continuación, configure la topología para que el trabajo se descargue a un trabajador dedicado.
