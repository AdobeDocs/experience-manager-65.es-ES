---
title: AEM Monitorización y mantenimiento de la instancia de
...
# AEM Monitorización y mantenimiento de la instancia de{#monitoring-and-maintaining-your-aem-instance}

AEM Una vez implementadas las instancias de su, se necesitarán ciertas tareas para monitorizar y mantener su funcionamiento, rendimiento e integridad.

Un factor clave aquí es que, para reconocer posibles problemas, debe saber cómo se ven y comportan sus sistemas en condiciones normales. La mejor manera de hacerlo es monitorizar el sistema y recopilar información durante un periodo de tiempo.
...
| Se están monitorizando los sistemas de archivos. | El repositorio CRX se &quot;bloqueará&quot; si no hay suficiente espacio en disco. Se reanudará una vez que el espacio esté disponible. | &quot; `*ERROR* LowDiskSpaceBlocker`Los mensajes &quot; se pueden ver en el archivo de registro cuando hay poco espacio disponible. |
...
Es probable que su compañía tenga una directiva de copia de seguridad que deberá seguir, consideraciones adicionales sobre qué copia de seguridad y cuándo incluir:

* qué tan críticos son el sistema y los datos.
* la frecuencia con la que se realizan cambios en el software o en los datos.
* volumen de datos; la capacidad puede ser ocasionalmente un problema, al igual que el tiempo necesario para realizar la copia de seguridad.
...
* su política de recuperación ante desastres; ¿existen directrices sobre dónde deben almacenarse los datos de copia de seguridad (por ejemplo, fuera del sitio, medio específico, etc.)?

A menudo, se realiza una copia de seguridad completa a intervalos regulares (por ejemplo, diariamente, semanalmente o mensualmente), con copias de seguridad incrementales intermedias (por ejemplo, cada día o semanalmente).

>[!CAUTION]
>
>Al implementar copias de seguridad de las instancias de producción, realice pruebas *debe* para garantizar que la copia de seguridad se pueda restaurar correctamente.
>
...
Después de la instalación, o de realizar cambios importantes en la configuración, realice una copia de seguridad de la instalación del software.

Para ello, debe hacer lo siguiente [realizar una copia de seguridad de todo el repositorio](#backing-up-your-repository) y luego:
...
>[!CAUTION]
>
>Si utiliza un servidor de aplicaciones de terceros, es posible que haya carpetas adicionales en una ubicación diferente y también sea necesario realizar una copia de seguridad de las mismas. Consulte [AEM Cómo instalar el servidor de aplicaciones con un servidor de aplicaciones de manera que se pueda instalar](/help/sites-deploying/application-server-install.md) para obtener información sobre la instalación de servidores de aplicaciones.
...
1. Seleccionar **Depuración de versión** de la lista desplegable en la **Añadir nueva tarea** diálogo. Entonces **Guardar**.

   ![Agregar purga de versión](assets/version-purge-add-new-task.png)

1. El **Depuración de versión** se agregará la tarea. Utilice las acciones de tarjeta para lo siguiente:
...
1. Seleccione el **Configurar** acción para abrir la consola web de **Tarea de depuración de versión de CQ WCM por día**, donde puede configurar:
   ...
   * **Purga de rutas**
Defina la ruta de inicio del contenido que se va a depurar; por ejemplo, 
`/content/wknd`.

      >[!CAUTION]
      >
      >Se recomienda encarecidamente que defina varias rutas para cada uno de los sitios web.
      >
      >La definición de una ruta con demasiados hijos puede alargar significativamente el tiempo necesario para realizar la depuración.
...
>
>Los nodos purgados no se pueden revertir sin restaurar el repositorio. Debe encargarse de la configuración, por lo que le recomendamos que realice siempre una ejecución en seco antes de purgar.
...
* `V 1.0.1`&#42;: el asterisco indica que la versión es la versión actual (base) y que no se puede purgar.

* `Thu Mar 15 2012 08:37:32 GMT+0100`: la fecha de la versión.

En el siguiente ejemplo:

* El **[!DNL Shirts]** Las versiones de se purgan porque su antigüedad es bueno a 2 días.
* El **[!DNL Tonga Fashions!]** Las versiones se purgan porque su número de versiones es bueno a 5.

![global_version_screenshot](assets/global_version_screenshot.png)

## Trabajar con registros de auditoría y archivos de registro {#working-with-audit-records-and-log-files}

Los registros de auditoría y los archivos de registro relacionados con Adobe Experience Manager AEM () se encuentran en varias ubicaciones. A continuación se proporciona una descripción general de lo que puede encontrar donde.
...
La rotación del archivo de registro hace referencia al proceso que limita el crecimiento del archivo al crear un nuevo archivo periódicamente. AEM En, un archivo de registro llamado: `error.log` se rotarán una vez al día según las reglas dadas:

* El `error.log` se ha cambiado el nombre del archivo según el patrón {original_filename} `.yyyy-MM-dd`. Por ejemplo, el 11 de julio de 2010, se cambia el nombre del archivo de registro actual a `error.log-2010-07-10`, luego un nuevo `error.og` se ha creado.
...
>[!NOTE]
>
>AEM AEM Si actualiza la instalación de la, tenga en cuenta que cualquier archivo de registro existente que ya no utilice la instalación permanecerá en el disco. Puede eliminarlos sin riesgo. Todas las nuevas entradas de registro se escribirán en los nuevos archivos de registro.
...
* `<cq-installation-dir>/crx-quickstart/logs`
   ...
   * `access.log`
AEM Todas las solicitudes de acceso a WCM y al repositorio de la se registran aquí.
...
Este registro solo se utiliza si [!DNL Dynamic Media] está activada. El registro de acceso de s7 registra cada solicitud realizada a [!DNL Dynamic Media] mediante `/is/image` y `/is/content`.
   ...
   * `stderr.log`
Contiene mensajes de error, de distinto nivel de gravedad, generados durante el inicio. De forma predeterminada, el nivel de registro se establece en 
`Warning` ( `WARN`)
...
>[!NOTE]
>
>Los registros de acceso de ImageServer y s7 no se incluyen en el paquete **Descargar ** completo&quot; generado desde la página **system/console/status-Bundlelist **. Para fines de soporte, si tiene [!DNL Dynamic Media] En caso de problemas, anexe también los registros de acceso de ImageServer y s7 cuando se ponga en contacto con Atención al cliente.
...
>[!CAUTION]
>
>No deje el registro en el nivel de registro de depuración más tiempo del necesario, ya que genera muchas entradas de registro y consume recursos.
...
|---|---|---|
| 1 | Error | La acción ha fallado. AEM La instalación continúa, pero una parte de WCM no se instaló correctamente y no funcionará. |
...
En determinadas circunstancias, es posible que desee crear un archivo de registro personalizado con un nivel de registro diferente. Para ello, en el repositorio, haga lo siguiente:

1. Si no existe todavía, cree una nueva carpeta de configuración ( `sling:Folder`) para su proyecto `/apps/<project-name>/config`.
...
1. Este paso sólo es necesario cuando se requiere un nuevo Writer (es decir, con una configuración distinta de la predeterminada Writer).
   ...
   >
   >Si no se ha configurado ningún objeto Writer explícito, el sistema generará automáticamente un objeto Writer implícito basado en el valor predeterminado.
   ...
   * Nombre: `org.apache.sling.commons.log.LogManager.factory.writer-<identifier>` (como esto es un escritor)
   ...
   * Configure los demás parámetros según sea necesario:
   ...
   >
   >para indicar cuándo se creará un nuevo archivo (y el nombre del archivo existente se cambiará según el patrón de nombre).
   >
   ...
   >* Se puede especificar una programación de fecha y hora como `java.util.SimpleDateFormat` patrón. Define el período de tiempo después del cual se girará el archivo; también el sufijo anexado al archivo girado (para su identificación).

   >
   >El valor predeterminado es &#39;.&#39;dd/MM/yyyy (para la rotación diaria del registro).
   >
   >Por ejemplo, a medianoche del 20 de enero de 2010 (o cuando el primer mensaje de registro posterior sea preciso), ../logs/error.log pasará a denominarse ../logs/error.log.2010-01-20. El registro del 21 de enero se generará en (un nuevo y vacío) ../logs/error.log hasta que se renueve al siguiente cambio de día.
   >
...
1. Abra una página.
1. En la barra de tareas, puede seleccionar la pestaña con el icono de candado y, a continuación, hacer doble clic en **Registro de auditoría...**
1. Se abrirá una nueva ventana que mostrará la lista de registros de auditoría de la página actual.
...
* si el tamaño de la cola aumenta con el tiempo, esto puede indicar una cola bloqueada.
...
1. Haga clic en el nombre del agente apropiado (que es un vínculo) para mostrar información detallada sobre ese agente:
   ...
   * **Actualizar** o **Borrar** para actualizar la visualización de las entradas de la cola; esto le ayuda a ver los elementos entrar y salir de la cola.
   ...
   * **Forzar reintento** en cualquier elemento en cola si es necesario.
   ...
   >
   >Si se realiza una prueba de replicación para una cola de Bandeja de salida, los elementos anteriores a la replicación de prueba se volverán a procesar con cada replicación inversa.
   >
   >Si estos elementos ya existen en cola, se pueden encontrar con la siguiente consulta JCR XPath y deben eliminarse.
   >
...
[Optimización del rendimiento](/help/sites-deploying/configuring-performance.md) es un proceso interactivo que recibe atención durante el desarrollo. Después de la implementación, generalmente se revisa después de intervalos o eventos específicos.
...
| Aplicación | El uso de CPU del servidor es alto. | AEM Clúster las instancias de la. | Busque y elimine los contenedores de CPU y memoria (utilice la revisión del código, la salida del tiempo, etc.). |
...
Los problemas de rendimiento pueden deberse a una serie de causas que no tienen nada que ver con su sitio web, incluidas las ralentizaciones temporales en la velocidad de conexión, la carga de la CPU y muchas más.
...
* Cuando experimenta un problema de rendimiento:
   ...
   * intente replicarlo con uno (o preferiblemente más) navegadores web estándar, en un cliente diferente que sepa que tiene un buen rendimiento general y/o en el propio servidor (si es posible)
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>Comandos Unix/Linux para rastrear llamadas y señales del sistema. Aumente el nivel de registro a <code>INFO</code>.</p> <p>Analice la cantidad de cargas de página por solicitud, qué páginas, etc.</p> </td>
  </tr>
  <tr>
   <td>Volcados de hilos</td>
   <td>Observe los hilos de JVM. Identificar contenciones, bloqueos y errores de larga duración.</td>
  ...
  <tr>
   ...
   <td><p>Añada el:<br /> <code>-XX:+HeapDumpOnOutOfMemoryError</code><br /> AEM a la llamada de java a la función de llamada a la.</p> <p>Consulte la <a href="https://docs.oracle.com/javase/8/docs/technotes/guides/troubleshoot/prepapp002.html#CEGBHDFH">Página Opciones/Indicadores para Resolución de Problemas de JVM</a>.</p> </td>
  </tr>
  <tr>
   <td>Llamadas del sistema</td>
   <td>Identificar problemas de temporización.</td>
   <td><p>Llamadas a <code>System.currentTimeMillis()</code> o <code>com.day.util</code>.Timestamps se utilizan para generar marcas de tiempo a partir del código o a través de <a href="#html-comments">HTML-comentarios</a>.</p> <p><strong>Nota:</strong> Estos deben implementarse para que se puedan activar o desactivar según sea necesario; cuando un sistema funcione sin problemas, no se necesitará la sobrecarga de recopilar estadísticas.</p> </td>
  </tr>
  <tr>
   <td>Apache Bench</td>
  ...
  <tr>
   ...
   <td>Llamada al núcleo en profundidad y análisis de procesos (Unix).</td>
  ...
  <tr>
   ...
   <td>Por ejemplo, <a href="https://www.yourkit.com/">Su kit</a>. o el <a href="https://docs.oracle.com/javase/8/docs/technotes/guides/troubleshoot/tooldescr004.html#BABJJEEE">Grabador de vuelo Java</a>.</td>
  </tr>
  <tr>
   <td><a href="#information-collection">Recopilación de información</a></td>
   <td>El estado actual de la instalación.</td>
   <td>Conocer en la medida de lo posible su instalación también puede ayudarle a rastrear qué podría haber causado un cambio en el rendimiento y si estos cambios están justificados. Estas métricas deben recopilarse a intervalos regulares para que pueda ver fácilmente cambios significativos.</td>
  </tr>
 </tbody>
</table>
...
El `request.log` ofrece una forma integrada de obtener información sobre cuánto tiempo tardan las solicitudes. A efectos de desarrollo, resulta útil `tail -f` el `request.log` y observe los tiempos de respuesta lentos. Para analizar un `request.log` recomendamos el [uso de `rlog.jar` que le permite ordenar y filtrar los tiempos de respuesta](#using-rlog-jar-to-find-requests-with-long-duration-times).

Se recomienda aislar las páginas &quot;lentas&quot; del `request.log`y luego ajustarlos individualmente para obtener un mejor rendimiento. Esto se suele hacer incluyendo métricas de rendimiento por componente o utilizando una herramienta de perfil de rendimiento como ` [yourkit](https://www.yourkit.com/)`.
...
Al sumar todas las entradas de GET dentro de períodos específicos (por ejemplo, en varios períodos de 24 horas) puede realizar declaraciones sobre el tráfico promedio en su sitio web.
...
Mediante pequeños scripts, puede extraer la información necesaria del archivo de registro y combinar las estadísticas que desee. Desde estas, puede ver qué páginas o tipos de páginas son lentos y si el rendimiento general es satisfactorio.
...
Sin embargo, una vez que haya determinado el tiempo de respuesta, es posible que tenga que analizar por qué la solicitud está tardando tanto tiempo y qué se puede hacer para mejorar la respuesta.
...
* monitorice cuántas solicitudes se realizan en un lapso de tiempo específico, por ejemplo: un minuto
* probar los efectos de un número específico de usuarios que realizan las mismas solicitudes al mismo tiempo (lo más cerca posible); por ejemplo, 30 usuarios haciendo clic en **Guardar** al mismo tiempo.
...
AEM La incluye varias herramientas de ayuda ubicadas en:
`<cq-installation-dir>/crx-quickstart/opt/helpers`

Uno de estos, `rlog.jar`, se puede utilizar para ordenar rápidamente `request.log` de modo que las solicitudes se muestren por duración, desde la más larga a la más corta.
...
Por ejemplo, puede ejecutarlo especificando `request.log` como parámetro y mostrar las 10 primeras solicitudes que tienen la duración más larga:
...
Es posible que tenga que concatenar al individuo `request.log` archivos si necesita realizar esta operación en una muestra de datos de gran tamaño.

### Apache Bench {#apache-bench}

Para minimizar el impacto de casos especiales (como la recolección de basura, etc.), se recomienda utilizar una herramienta como `apachebench` (consulte, por ejemplo, [ab](https://httpd.apache.org/docs/2.4/programs/ab.html) para obtener más documentación) para ayudar a identificar las fugas de memoria y analizar selectivamente el tiempo de respuesta.
...
AEM Los números anteriores se toman de un portátil MAcBook Pro estándar (mediados de 2010) que accede a la página de la empresa de geometrixx, como se incluye en una instalación predeterminada de la. La página es muy sencilla, pero no está optimizada para el rendimiento.

`apachebench` también muestra el tiempo por solicitud como media en todas las solicitudes simultáneas; consulte `Time per request: 54.595 [ms]` (media, en todas las solicitudes simultáneas). Puede cambiar el valor del parámetro de concurrencia `-c` (número de solicitudes múltiples que se realizan a la vez) para ver cualquier efecto.

### Contadores de solicitudes {#request-counters}

La información sobre el tráfico de la solicitud (número de solicitudes durante un período de tiempo específico) le proporciona una indicación de la carga de la instancia. Esta información se puede extraer de [request.log](#interpreting-the-request-log), aunque el uso de contadores automatizará la recopilación de datos para que pueda ver lo siguiente:
...
Se recomienda que cada proyecto incluya `html comments` para el rendimiento del servidor. Se pueden encontrar muchos ejemplos públicos buenos; seleccione una página, abra el origen de la página para verlo y desplácese hasta la parte inferior; se puede ver código como el siguiente:
...
1. Desde dentro de `Local` aplicación, doble clic `com.day.crx.quickstart.Main`; la Información general se mostrará como predeterminada:

   ![chlimage_1-1](assets/chlimage_1-1.png)

   A continuación, puede seleccionar otras opciones.

### Monitorización del rendimiento con (J)VisualVM {#monitoring-performance-using-j-visualvm}

Para JDK 6-8, el comando tool `visualvm` está disponible. Una vez instalado un JDK, puede hacer lo siguiente:
...
1. Desde dentro de `Local` aplicación, doble clic `com.day.crx.quickstart.Main`; la Información general se mostrará como predeterminada:
   ...
   A continuación, puede seleccionar otras opciones, como Supervisar:
...
Conocer en la medida de lo posible su instalación puede ayudarle a averiguar qué puede haber causado un cambio en el rendimiento y si estos cambios están justificados. Estas métricas deben recopilarse a intervalos regulares para que pueda ver fácilmente cambios significativos.
...
Para determinar las solicitudes por hora que tiene en el sistema de creación en la hora pico:
...
>
>* [Volcados de hilos](https://helpx.adobe.com/experience-manager/kb/TakeThreadDump.html)
...
Si el sistema se está quedando sin memoria, esto se puede ver de varias maneras, incluida la degradación del rendimiento y los mensajes de error, incluido el subtexto:
...
Si su sistema se está quedando sin espacio en disco, o si nota que se están iniciando las pulsaciones del disco, consulte:

* Si ha deshabilitado la recopilación de información de depuración; esto se puede configurar en varias ubicaciones, entre ellas:
   ...
   * [Administrador de biblioteca del HTML de CQ](/help/sites-deploying/osgi-configuration-settings.md#daycqhtmllibrarymanager)
...
* Si ha configurado y cómo [Depuración de versiones](/help/sites-deploying/version-purging.md)
* Base de conocimiento:
...
La Máquina virtual Java (JVM) ha mejorado significativamente en lo que respecta al ajuste (especialmente desde Java 7). Debido a esto, a menudo será adecuado especificar un tamaño de JVM fijo razonable y utilizar los valores predeterminados.

Si la configuración predeterminada no es adecuada, es importante establecer un método para monitorizar y evaluar el rendimiento de GC antes de intentar ajustar la JVM; esto puede implicar factores de monitorización, como el tamaño de la pila, el algoritmo y otros aspectos.
...
* A continuación, conéctese a JVM con JConsole; consulte:
   ` [https://docs.oracle.com/javase/8/docs/technotes/guides/management/jconsole.html](https://docs.oracle.com/javase/8/docs/technotes/guides/management/jconsole.html)`

Esto le ayudará a ver cuánta memoria se está utilizando, qué algoritmos GC se están utilizando, cuánto tiempo tardan en ejecutarse y qué efecto tiene en el rendimiento de su aplicación. Sin esto, la sintonización es solo &quot;perillas giratorias aleatoriamente&quot;.
