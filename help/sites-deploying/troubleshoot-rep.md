...
1. Vaya a `/crx/explorer` y entre como `admin`.
...
Para comprobarlo, vaya a `/etc/replication/agents.author.html` y haga clic en los agentes de replicación.
...
1. ¿Muestra el estado de la cola **La cola está activa - N.º pendiente**? El trabajo de replicación se pudo quedar atascado en una lectura de socket a la espera de que la instancia de publicación o Dispatcher respondan. Esto puede significar que la instancia de publicación o Dispatcher está bajo carga alta o atascada en un bloqueo. Tome los volcados de hilos del autor y publíquelos en este caso.
   ...
   * Abra los volcados de hilos de la publicación en un analizador de volcado de hilos y analice qué podría estar causando que la instancia de publicación no responda. Debería ver un subproceso con POST `/bin/receive` en su nombre. Ese es el hilo que recibe la replicación del autor.
...
1. Es posible que un fragmento de contenido determinado no se pueda serializar en /var/replication/data debido a que el repositorio está dañado o a algún otro problema. Compruebe si hay algún error relacionado en logs/error.log. Para borrar el elemento de replicación incorrecto, haga lo siguiente:
   ...
   1. Vaya a `https://<host>:<port>/crx/de` y entre como usuario administrador.
...
1. Puede ser que el procesamiento del trabajo esté desactivado. Puede comprobarlo en la Consola Felix, en la pestaña Evento de Sling. Compruebe si se muestra: Evento de Apache Sling (EL PROCESAMIENTO DEL TRABAJO ESTÁ DESHABILITADO)
   ...
   * Si es así, compruebe el controlador de eventos del trabajo Apache Sling en la pestaña Configuración en la consola Felix. La casilla &quot;Procesamiento de trabajo activado&quot; no se ha podido desactivar. Si está activado y sigue mostrando que &quot;el procesamiento del trabajo está deshabilitado&quot;, compruebe si hay alguna superposición en `/apps/system/config` que esté deshabilitando el procesamiento del trabajo. Intente crear un nodo `osgi:config` para `jobmanager.enabled` con un valor booleano de `true` y vuelva a comprobar si se inició la activación y no hay más trabajos en cola.

1. También puede ocurrir que la configuración de DefaultJobManager pase a un estado incoherente. Esto puede suceder cuando alguien modifica manualmente la configuración del &quot;Controlador de eventos del trabajo de Apache Sling&quot; a través de la consola OSGi (por ejemplo, deshabilita y vuelve a habilitar la propiedad &quot;Procesamiento de trabajos habilitado&quot; y guarda la configuración).
   ...
   * Para resolver este problema, vaya a la página Configuración de la consola OSGi y elimine la configuración &quot;Controlador de eventos del trabajo Apache Sling&quot;. A continuación, reinicie el nodo principal del clúster para volver a poner la configuración en un estado coherente. Esto debería solucionar el problema y hacer que la replicación vuelva a funcionar.
...
1. Vaya a `https://<host>:<port>/system/console/configMgr` e inicie sesión como usuario administrador.
...
1. Establezca la configuración de esta manera:
   ...
   * Ruta de archivo de registro: `logs/replication.log`
   * Categorías: `com.day.cq.replication`

1. Si sospecha que el problema está relacionado con eventos/trabajos de sling de alguna manera, también puede agregar este paquete Java; en `categories:org.apache.sling.event`.

## Pausa de cola del agente de replicación  {#pausing-replication-agent-queue}

A veces puede ser adecuado pausar la cola de replicación para reducir la carga en el sistema de creación, sin deshabilitarla. Actualmente, esto solo es posible por un hackeo de la configuración temporal de un puerto no válido. A partir de la versión 5.4, se ha producido un botón de pausa en la cola del agente de replicación con algunas limitaciones:

1. El estado no persiste. Es decir, si reinicia un servidor o se recicla un paquete de replicación, vuelve al estado de ejecución.
1. La pausa está inactiva durante un período más corto (OOB 1 hora después de que no haya actividades con replicación de otros subprocesos). Esto se debe a que hay una función en Sling que evita los hilos inactivos. Compruebe si un subproceso de cola de trabajos ha estado sin utilizar durante mucho tiempo. Si es así, comienza ciclos de limpieza. Debido al ciclo de limpieza, detiene el subproceso y, por lo tanto, se pierde el ajuste en pausa. Como los trabajos se mantienen, inicia un nuevo subproceso para procesar la cola, que no tiene los detalles de la configuración en pausa. Debido a esto, la cola entra en estado de ejecución.
...
1. Inicie sesión en CRX/DE (`https://<host>:<port>/crx/de/index.jsp`) como administrador.
