...
>
>En caso de que esté leyendo este artículo como preparación para una actualización de un **AEM 5.x** instalación, asegúrese de consultar la [actualización](https://experienceleague.adobe.com/docs/) en primer lugar.
...
Por ejemplo, si el proyecto requiere muchos recursos multimedia, almacenarlos en el File o S3 Data Store hace que el acceso a ellos sea más rápido que almacenarlos directamente en un MongoDB.
...
| maxCachedBinarySize | Los binarios con un tamaño menor o igual que este valor (en bytes) se almacenan en la caché de memoria. | 17408 (17 KB) | No. |
...
| proxyHost | Establezca el host proxy opcional a través del cual se conecta el cliente. |  | No. |
| proxyPort | Establezca el puerto proxy opcional a través del que se conecta el cliente. |  | No. |
...
La caché local se comprueba para el registro del archivo o blob solicitado antes de acceder a él desde el DataStore. Cuando la caché exceda el límite configurado (consulte la `cacheSize` ) al agregar un archivo a la caché, algunos de los archivos se desalojan para recuperar espacio.
...
1. Si la instancia se ha clonado de un servidor existente, debe quitar la variable `clusterId` de la nueva instancia utilizando la última herramienta oak-run mientras el repositorio está sin conexión. El comando que debe ejecutar es:
   ...
   >
   >
   >Se deben utilizar distintas versiones de la herramienta en función de la versión de Oak que utilice con la instalación de AEM. Compruebe la lista de requisitos de la versión que aparece a continuación antes de utilizar la herramienta:
   >
   >
   >
...
1. Finalmente, valide la configuración. Para validar, busque un archivo único agregado al almacén de datos por cada repositorio que lo comparte. El formato de los archivos es `repository-[UUID]`, donde el UUID es un identificador único de cada repositorio individual.
...
AEM puede configurarse para almacenar datos en el servicio de almacenamiento Azure de Microsoft®. Utiliza el `org.apache.jackrabbit.oak.plugins.blob.datastore.AzureDataStore.config` PID para configuración.

Para habilitar la funcionalidad del almacén de datos de Azure, se debe descargar e instalar un paquete de características que contenga el conector de Azure. Vaya a la [Repositorio de Adobes](https://repo1.maven.org/maven2/com/adobe/granite/com.adobe.granite.oak.azureblobconnector/) y descargue la versión más reciente de las versiones 1.6.x del feature pack (por ejemplo, com.adobe.granite.oak.azureblobconnector-1.6.3.zip).

>[!NOTE]
>
>Al utilizar AEM con TarMK, los binarios se almacenan de forma predeterminada en FileDataStore. Para usar TarMK con el DataStore de Azure, debe empezar AEM usar la variable `crx3tar-nofds` runmode, por ejemplo:
...
* azureSas=&quot;&quot;: En la versión 1.6.3 del conector, se agregó compatibilidad con Azure Shared Access Signature (SAS). **Si tanto SAS como credenciales de almacenamiento existen en el archivo de configuración, SAS tiene prioridad.** Para obtener más información sobre SAS, consulte [documentación oficial](https://learn.microsoft.com/en-us/azure/storage/common/storage-sas-overview). Asegúrese de que el carácter &#39;=&#39; se escapa como &#39;\=&#39;.
...
* maxCachedBinarySize: Los binarios con un tamaño menor o igual que este tamaño se almacenan en la caché de memoria. El tamaño se expresa en bytes. El valor predeterminado es 17408 (17 KB).
...
1. En el cuadro de diálogo siguiente, introduzca `false` para el `markOnly` y, a continuación, haga clic en **Invocar**:
   ...
   >[!NOTE]
   >
   >La variable `markOnly` significa si la fase de barrido de la colección de residuos se ejecuta o no.
...
>[!NOTE]
>
>Al realizar la colección de residuos en un almacén de datos agrupado o compartido, la configuración (con Mongo o Segment Tar) del registro podría mostrar advertencias sobre la incapacidad de eliminar ciertos ID de blob. Los ID de blob eliminados en una colección de residuos anterior vuelven a ser incorrectamente referenciados por otros clústeres o nodos compartidos que no tienen información sobre las eliminaciones de ID. Como resultado, cuando se realiza la colección de residuos, registra una advertencia cuando intenta eliminar un ID que ya se ha eliminado en la última ejecución. Este comportamiento no afecta al rendimiento ni a la funcionalidad.
...
1. Después de completar el procedimiento anterior en todas las instancias, ejecute de nuevo la colección de residuos del almacén de datos desde **any** de las instancias:
   ...
   Todos los archivos encontrados se cotejan utilizando la fase de marca utilizada antes y eliminan el resto que no se utiliza del almacén de datos.
