...
* Se instaló la plataforma Java™, Standard Edition JDK u otro soporte [Máquinas virtuales Java™](#java-virtual-machines)
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Para garantizar el éxito de los proyectos de los clientes, Adobe proporciona compatibilidad total dentro de un programa de asistencia restringido, que requiere que se cumplan las condiciones específicas. El soporte de nivel R requiere una solicitud formal del cliente y confirmación por parte del Adobe. Para obtener más información, póngase en contacto con el servicio de atención al cliente de Adobe.</td>
  </tr>
 </tbody>
</table>
...
|---|---|
| **Z: No compatible** | La configuración no es compatible. Adobe no hace afirmaciones sobre si la configuración funciona y no la admite. |

## Plataformas compatibles {#supported-platforms}

### Máquinas virtuales Java™ {#java-virtual-machines}

La aplicación requiere una máquina virtual Java™ para ejecutarse, que se proporciona en la distribución Java™ Development Kit (JDK).

Adobe Experience Manager funciona con las siguientes versiones de las máquinas virtuales Java™:

>[!CAUTION]
>
>Rastree los boletines de seguridad del proveedor Java™. Al hacerlo, se garantiza la seguridad de los entornos de producción. Además, instale siempre las últimas actualizaciones de Java™.
...
1. Oracle se ha trasladado a un modelo de &quot;compatibilidad a largo plazo&quot; (LTS) para los productos Java™ SE de Oracle. Java™ 9, Java™ 10 y Java™ 12 son versiones no LTS por Oracle (consulte [Hoja de ruta de compatibilidad con Java™ SE de oracle](https://www.oracle.com/technetwork/java/eol-135779.html)). Para implementar AEM en el entorno de producción, Adobe solo es compatible con las versiones LTS de Java™. La compatibilidad y distribución del Oracle Java™ SE JDK, incluidas todas las actualizaciones de mantenimiento de las versiones LTS más allá del final de las actualizaciones públicas, es compatible directamente con el Adobe para todos los clientes AEM que utilizan la tecnología Oracle Java™ SE. Consulte la [Política de soporte de Java™ para Adobe Experience Manager](assets/Java_Policy_for_Adobe_Experience_Manager.pdf).
   **Importante: Oracle Java™ 11 es compatible hasta septiembre de 2026 como mínimo. Se está preparando la compatibilidad con el Oracle Java™ 17.**

1. El IBM® JRE solo es compatible con el servidor de aplicaciones WebSphere®.

1. Las versiones LTS de Azul Zulu OpenJDK son compatibles con implementaciones de AEM locales a partir de la versión 6.5 SP9. El soporte y la distribución de las versiones de Azul Zulu JDK LTS deben ser licenciados directamente desde Azul por clientes de Adobe.
...
| Microsoft® SQL Server 2016 | Base de datos de Forms | A: Compatible |
| **Apache Lucene (integrado de Quickstart)** | Servicio de búsqueda | A: Compatible |
| Apache Solr | Servicio de búsqueda | A: Compatible |

1. &#39;File System&#39; incluye almacenamiento en bloque compatible con POSIX. Incluye tecnología de almacenamiento en red. Tenga en cuenta que el rendimiento del sistema de archivos puede variar e influir en el rendimiento general. Cargue AEM de prueba con el sistema de archivos remoto/de red.
...
>[!NOTE]
MongoDB es software de terceros y no está incluido en el paquete de licencias de AEM. Para obtener más información, consulte la [Política de licencias de MongoDB](https://www.mongodb.com/community/licensing) página.
...
Si desea ejecutar el autor y la publicación en MongoDB, se deben adquirir dos licencias independientes.
El Servicio de atención al cliente de Adobe ayuda a resolver los problemas relacionados con el uso de MongoDB con AEM.
...
Para ejecutar AEM 6.5 con una base de datos relacional admitida, se requiere un contrato de soporte independiente con un proveedor de base de datos. El Servicio de atención al cliente de Adobe ayuda a resolver los problemas relacionados con el uso de bases de datos relacionales con AEM 6.5.
...
Adobe Experience Manager es compatible con la ejecución en una máquina virtual en entornos de computación en la nube. Estos entornos incluyen Microsoft® Azure y Amazon Web Service (AWS), que se ejecutan de acuerdo con los requisitos técnicos enumerados en esta página y según los términos de asistencia estándar de Adobe.
...
1. Los servidores web basados en el código fuente httpd de Apache tienen tanta compatibilidad como la versión de httpd en la que se basa. En caso de duda, pida al Adobe que confirme el nivel de asistencia relacionado con el producto del servidor correspondiente. Los siguientes casos:

   1. El servidor HTTP se creó utilizando solamente las distribuciones de fuentes oficiales de Apache, o
   1. El servidor HTTP se entregó como parte del sistema operativo en el que se está ejecutando. Ejemplos: Servidor HTTP IBM®, servidor HTTP de Oracle
...
1. Versión ampliada de soporte de Firefox [Más información sobre mozilla.org](https://www.mozilla.org/en-US/firefox/enterprise/)
...
Cuando se conecta con Microsoft® Windows 7+ a una instancia de AEM que no está segura con SSL, la autenticación básica sobre una red no segura debe habilitarse en Windows. Requiere un cambio en el Registro de Windows de WebClient:
...
La operación es perfecta, ya que no se requiere ninguna configuración especial. Si es necesario, puede especificar una dirección IP con el formato apropiado para su tipo de red.

Cuando se debe especificar una dirección IP, se puede seleccionar (según sea necesario) entre las siguientes opciones:
...
Para los clientes de Dynamic Media que ejecutan Dynamic Media: modo Scene7 (es decir, **dynamic_media_scene7** modo de ejecución), no hay requisitos adicionales del sistema; solo los mismos requisitos del sistema que AEM. Dynamic Media: la arquitectura del modo Scene7 utiliza el servicio de imagen basado en la nube y no el servicio incrustado en AEM.
...
* Red Hat® Enterprise 7 o CentOS 7 y posteriores con los parches de correcciones más recientes
* Sistema operativo de 64 bits
* Intercambio desactivado (recomendado)
* SELinux deshabilitado (ver nota a continuación)

>[!NOTE]
Si la configuración regional está definida de tal modo que LC_CTYPE no sea igual a `en_US.UTF-8`, impide que Dynamic Media funcione. Para ver cuál es su valor, escriba &quot;locale&quot; en el símbolo del sistema. Si no está correctamente configurada, entonces establezca la variable de entorno LC_CTYPE en la cadena vacía escribiendo &quot;export LC_CTYPE=&quot; antes de ejecutar AEM.
...
>[!NOTE]
**Arquitectura NUMA:** Los sistemas con procesadores AMD64 e Intel® EM64T suelen configurarse como plataformas de arquitectura de memoria no uniforme (NUMA). Es decir, el núcleo construye varios nodos de memoria en el momento del arranque en lugar de construir un solo nodo de memoria.
...
* El generador de PDF no convierte los archivos mediante el proyecto Microsoft® 2019. Puede seguir utilizando Microsoft® Project 2016 para convertir `.VSD` y `.VSDX` archivos.
>
...
* **Sistemas operativos:**
   ...
   * Linux® (compatibilidad con aplicaciones de 32 y 32 bits en sistemas de 64 bits). Para ver los pasos para instalar bibliotecas de cliente de 32 bits, consulte [Cómo habilitar la extracción y escritura de XMP en Red Hat® Linux® de 64 bits](https://helpx.adobe.com/experience-manager/kb/enable-xmp-write-back-64-bit-redhat.html).
