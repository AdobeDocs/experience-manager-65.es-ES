...
description: AEM Obtenga información sobre cómo configurar y monitorizar los agentes de replicación en la.
...
* el autor solicita que se publique cierto contenido (activado); esto puede iniciarse mediante una solicitud manual o mediante déclencheur automáticos que se han preconfigurado.
* la solicitud se pasa al agente de replicación predeterminado adecuado; un entorno puede tener varios agentes predeterminados que siempre se seleccionarán para dichas acciones.
...
En algunos casos, se necesita un tipo de replicación conocido como replicación inversa para devolver estos datos al entorno de creación desde el que se redistribuyen a otros entornos de publicación. Debido a consideraciones de seguridad, cualquier tráfico de publicación al entorno de creación debe controlarse estrictamente.
...
AEM En otros casos, como en el caso de las funciones de las comunidades (por ejemplo, foros, blogs, comentarios y revisiones), la cantidad de contenido generado por el usuario (UGC) que se introduce en el entorno de publicación es difícil de sincronizar eficazmente entre instancias de mediante la replicación.
...
Para seguir este ejemplo y utilizar los agentes de replicación predeterminados, debe hacer lo siguiente [AEM Instalación de](/help/sites-deploying/deploy.md) con:
...
1. Edite la página para añadir texto nuevo.
...
Esta replicación la lleva a cabo desde el entorno de creación el:
...
Se utiliza para replicar del autor a la publicación.

* Vaciado de Dispatcher Se utiliza para administrar la caché de Dispatcher. Consulte [Invalidar la caché de Dispatcher desde el entorno de creación](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-the-authoring-environment) y [Invalidar la caché de Dispatcher desde una instancia de publicación](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-a-publishing-instance) para obtener más información.

* [Replicación inversa](#reverse-replication-publish-to-author)
Se utiliza para replicar desde la publicación al autor. La replicación inversa no se utiliza para funciones de Communities, como foros, blogs y comentarios. Se desactiva de forma efectiva, ya que la bandeja de salida no está activada. El uso de la replicación inversa requeriría una configuración personalizada.
...
Esto se solicitó para que cuando la página se solicite directamente desde el servidor de aplicaciones, se pueda ver el contenido. Es un agente especializado y (probablemente) no será necesario en la mayoría de los casos.
...
Para obtener información acerca de cómo conectar agentes de replicación a la instancia de publicación mediante MSSL, consulte [Duplicación mediante SSL mutuo](/help/sites-deploying/mssl-replication.md).
...
Desde la pestaña Herramientas del entorno de creación, puede configurar los agentes de replicación que residen en el entorno de creación (**Agentes en el autor**) o el entorno de publicación (**Agentes de publicación**). Los siguientes procedimientos ilustran la configuración de un agente para el entorno de creación, pero se puede utilizar para ambos.
...
1. Doble clic **Agentes en el autor** (panel izquierdo o derecho).
1. Haga clic en el nombre del agente apropiado (que es un vínculo) para mostrar información detallada sobre ese agente.
1. Clic **Editar** para abrir el cuadro de diálogo de configuración:
...
* un *bandeja de salida* en el entorno de publicación donde se coloca el contenido.
...
**Un agente de replicación inversa en el entorno de publicación (una bandeja de salida)** Este es el elemento pasivo, ya que actúa como &quot;bandeja de salida&quot;. La entrada del usuario se coloca aquí, desde donde la recopila el agente en el entorno de creación.
...
Tras la instalación, ya se ha configurado un agente predeterminado para la replicación de contenido en una instancia de publicación que se ejecuta en el puerto 4503 del localhost.
...
1. Seleccionar **Replicación**, entonces **Agentes en el autor** en el panel izquierdo.
1. Seleccionar **Nuevo...**.
1. Configure las variables **Título** y **Nombre**, luego seleccione **Agente de replicación**.
1. Clic **Crear** para crear el nuevo agente.
1. Haga doble clic en el nuevo elemento del agente para abrir el panel de configuración.
1. Clic **Editar** - el **Configuración de agente** se abrirá el cuadro de diálogo: **Tipo de serialización** ya está definido como Predeterminado, debe seguir siéndolo.
...
Las actualizaciones aparecerán en todas las instancias de publicación que se hayan configurado como anteriormente.
...
Los agentes predeterminados se incluyen en la instalación. Sin embargo, cierta configuración sigue siendo necesaria y lo mismo se aplica si está definiendo un nuevo agente:
...
1. Clic **Editar** - el **Configuración de agente** se abrirá el cuadro de diálogo:
...
>[!NOTE]
>
>La configuración de estos permisos no afectará a los usuarios que dupliquen contenido (por ejemplo, desde la consola Sitios web o la opción de la barra de tareas). El marco de replicación no utiliza la &quot;sesión de usuario&quot; del usuario actual para acceder a los agentes de replicación al replicar páginas.
...
Los dos `agents` mantenga la información de configuración sobre el entorno adecuado y solo estará activa cuando ese entorno se esté ejecutando. Por ejemplo, `agents.publish` solo se utilizará en el entorno de publicación. AEM La siguiente captura de pantalla muestra el agente de publicación en el entorno de creación, tal como se incluye con WCM de la:
...
A continuación, empaqueta todos los elementos de replicación en un paquete, que luego se replica como un solo archivo en el editor.

El editor desempaquetará todos los elementos, los guardará y responderá ante el autor.
