...
Por ejemplo, si se inició una instancia con los modos de ejecución `author,dev,emea`, y tanto `/apps/*/config.author/` como `/apps/*/config.emea.author/` define una configuración para`com.day.cq.wcm.core.impl.VersionManagerImpl`, se aplicará la configuración en `/apps/*/config.emea.author/` .
