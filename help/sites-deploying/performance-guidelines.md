...
Esta página proporciona directrices generales sobre cómo optimizar el rendimiento de la implementación de AEM. Si es nuevo en AEM, revise las páginas siguientes antes de comenzar a leer las directrices de rendimiento:
...
Utilice las directrices de rendimiento en las situaciones siguientes:

* **Implementación por primera vez**: Al planificar la implementación de AEM Sites o Assets por primera vez, es importante comprender las opciones disponibles. Especialmente al configurar el Micro Kernel, el Almacenamiento de nodos y el Almacén de datos (en comparación con la configuración predeterminada). Por ejemplo, cambiar la configuración predeterminada del almacén de datos para TarMK al almacén de datos de archivos.
...
* **Adición de más autores**: Cuando la topología TarMK recomendada no cumpla los requisitos de rendimiento y el tamaño del nodo Autor haya alcanzado la capacidad máxima disponible, comprenda las diferencias de rendimiento. Compárese con el uso de MongoMK con tres o más nodos Autor. Por ejemplo, implementar MongoMK en lugar de TarMK.
...
>[!CAUTION]
>
>El Micro Kernel de la Base de Datos Relacional está bajo soporte restringido. Contacto [Servicio de atención al cliente de Adobe](https://experienceleague.adobe.com/?support-solution=General&amp;support-tab=home&amp;lang=es#support) antes de utilizar este tipo de Micro Kernel.

![Chlimage_1-3](assets/chlimage_1-3a.png)

### Almacén de datos {#data-store}

Cuando se trata de un gran número de binarios, se recomienda utilizar un almacén de datos externo en lugar de los almacenes de nodos predeterminados para maximizar el rendimiento. Por ejemplo, si el proyecto requiere muchos recursos multimedia, almacenarlos en el Archivo o en el Almacén de datos de Azure/S3 hace que el acceso a ellos sea más rápido que almacenarlos directamente en un MongoDB.
...
>
>Para obtener recomendaciones sobre cómo implementar AEM en Azure o AWS, fuera de Adobe Managed Services, Adobe recomienda trabajar directamente con el proveedor de la nube. O bien, trabaje con uno de los socios de Adobe que admiten la implementación de AEM en el entorno de nube que elija. El proveedor o socio de nube seleccionado es responsable de las especificaciones de tamaño, el diseño y la implementación de la arquitectura que admiten para satisfacer sus requisitos específicos de rendimiento, carga, escalabilidad y seguridad.
...
>[!NOTE]
Para la mayoría de las implementaciones, Adobe recomienda utilizar el índice de Lucene. Utilice Solr solo para la escalabilidad en implementaciones especializadas y complejas.

![Chlimage_1-4](assets/chlimage_1-4a.png)

### Directrices de desarrollo {#development-guidelines}

Desarrollo para AEM que apunten a **rendimiento y escalabilidad**. A continuación se indican las prácticas recomendadas que puede seguir:
...
>[!NOTE]
Las directrices de arquitectura mínimas que se presentan a continuación son para entornos de producción y sitios de alto tráfico. Estas directrices son **not** el [especificaciones mínimas](/help/sites-deploying/technical-requirements.md#prerequisites) para ejecutar AEM.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Para evitar que las consultas expansivas sobrecarguen los sistemas, agregue estos parámetros de JVM en el script de inicio de AEM.</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><p>1048576 (1 MB) o menor</p> <p>2-10 % del tamaño máximo de pila</p> </td>
...
### Resultados de los indicadores de rendimiento {#performance-benchmark-results-1}

>[!NOTE]
Los números presentados a continuación se han normalizado a 1 como referencia y no son los números de rendimiento reales.

![imagen_1-10](assets/chlimage_1-10a.png) ![imagen_1-11](assets/chlimage_1-11a.png)
...
* **TarMK con File Datastore** - La arquitectura recomendada para la mayoría de los clientes:
...
* **MongoMK con File Datastore** - La arquitectura recomendada para la escalabilidad horizontal del nivel Autor:
...
* **La personalización de flujos de trabajo puede mejorar sustancialmente el rendimiento** - Elimine el paso de vídeo del flujo de trabajo &quot;Actualizar recurso&quot;, desactivando los oyentes que no se utilizan, etc.
