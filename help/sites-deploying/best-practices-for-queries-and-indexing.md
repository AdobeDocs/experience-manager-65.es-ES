...
Además, asegúrese de leer la [documentación de Oak sobre cómo escribir consultas e índices](/help/sites-deploying/queries-and-indexing.md). AEM Además de que los índices son un nuevo concepto en el 6, existen diferencias sintácticas en las consultas de Oak AEM que deben tenerse en cuenta al migrar código desde una instalación de anterior.
...
#### **Implementación de Post** {#post-deployment}
...
AEM *La reindexación siempre debe abordarse teniendo en cuenta su impacto para el rendimiento general de la y realizarse durante períodos de baja actividad o períodos de mantenimiento.*
...
AEM Si se experimenta un problema en el que no coincide con los criterios descritos a continuación, **no** vuelva a indexar ningún índice, ya que no resolverá el problema.
...
>[!NOTE]
>
>AEM En la versión 6.5, [oak-run.jar es el único método compatible](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree) para la reindexación en repositorios MongoMK o RDBMK.
...
>[!NOTE]
>
>La sección anterior resume y enmarca la guía de reindexación de Oak de la [documentación de Apache Oak AEM](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing) en el contexto de la.
...
* AEM Reindexando el procesamiento de **todo** contenido candidato en el repositorio; cuando los binarios de los que se extrae texto completo son numerosos o complejos, se coloca una mayor carga de cálculo para realizar la extracción de texto completo en los que se realiza la extracción de texto completo. Se ha hecho un mayor uso de los elementos de procesamiento de texto en el repositorio de texto. AEM AEM La preextracción de texto mueve el &quot;trabajo computacionalmente costoso&quot; de la extracción de texto a un proceso aislado que accede directamente al Almacén de datos de la, lo que evita la sobrecarga y la contención de recursos en los recursos de la.

AEM Se admite la implementación de un índice **new** lucene para que sea compatible con el índice de extracción binaria habilitado para la
...
AEM *El paso 2(a-c) se puede ejecutar durante el funcionamiento normal de ¿sólo interactúa con el almacén de datos?*.
...
El texto extraído previamente se identifica en el CSV mediante la huella digital binaria. AEM Si el archivo binario es el mismo, se puede utilizar el mismo texto extraído previamente en todas las instancias de la. AEM Dado que Publish AEM AEM AEM suele ser un subconjunto de Autor de la, el texto extraído previamente de Autor también se puede utilizar para reindexar también el texto extraído de la misma (suponiendo que el sistema de archivos de Publish AEM también tenga acceso a los archivos de texto extraídos, en el supuesto de que el sistema de archivos de Publish también tenga acceso a los archivos de texto extraídos).
...
3 bis. AEM [Reindex](#how-to-re-index) de los índices Lucene se invoca en el.
