...
Para mejorar los tiempos de carga de recursos, utilice almacenamiento de alto rendimiento para el directorio temporal de Java. En Linux y Windows, se puede utilizar una unidad RAM o SSD. En entornos basados en la nube, se podría utilizar un tipo de almacenamiento de alta velocidad equivalente. Por ejemplo, en Amazon EC2, se puede utilizar una unidad [&#39;efímera&#39;](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/InstanceStorage.html) para la carpeta temporal.
...
En el sistema operativo Windows, utilice un controlador de terceros para crear una unidad de RAM o simplemente utilice almacenamiento de alto rendimiento como SSD.
...
1. Seleccione Flujo de trabajo **[!UICONTROL transitorio]** y haga clic en **[!UICONTROL Aceptar]**.
...
Normalmente, ejecute los flujos de trabajo de depuración semanalmente. Sin embargo, en situaciones de uso intensivo de recursos, como durante la ingestión de recursos a gran escala, puede ejecutarlo con más frecuencia.
...
Además, establezca la ruta de la carpeta temporal de ImageMagick en el `configure.xml` archivo (o estableciendo la variable de entorno `MAGIC_TEMPORARY_PATH`) en una partición de disco que tenga suficiente espacio e IOPS.
...
1. Agregue una propiedad String[] `excludedPaths` con valores `/var`, `/etc/workflow/instances`y `/etc/replication`.
1. Vaya a `/oak:index/damAssetLucene`. Agregue una `String[]` propiedad `includedPaths` con valor `/content/dam`.
1. Guardar.
...
Para minimizar la latencia y lograr un alto rendimiento mediante una utilización eficiente de la CPU y el uso compartido de la carga, supervise el rendimiento de su instancia de AEM de forma regular. En particular:
