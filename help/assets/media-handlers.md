...
Se puede configurar un flujo de trabajo para que se ejecute automáticamente cuando se carga un recurso de un tipo MIME en particular. Los pasos de procesamiento se definen en términos de una serie de [!DNL Assets] controladores de medios. [!DNL Experience Manager] proporciona algunas [controladores integrados,](#default-media-handlers) y las adicionales pueden ser [desarrollo personalizado](#creating-a-new-media-handler) o se definen delegando el proceso a un [herramienta de línea de comandos](#command-line-based-media-handler).
...
Los siguientes controladores de medios están disponibles en [!DNL Assets] y gestione los tipos MIME más comunes:
...
1. En el navegador, vaya a `https://localhost:4502/system/console/components`.
...
[!DNL Experience Manager] tiene algunos flujos de trabajo predeterminados para procesar recursos. Para visualizarlos, abra la consola Flujo de trabajo y haga clic en el botón **[!UICONTROL Modelos]** pestaña: los títulos del flujo de trabajo que empiezan por [!DNL Assets] son los recursos específicos.
...
Consulte [Herramientas de desarrollo](../sites-developing/dev-tools.md) para instalar y configurar Eclipse con un [!DNL Maven] y para configurar las dependencias necesarias para la variable [!DNL Maven] proyecto.
...
1. En Eclipse, cree `myBundle` [!DNL Maven] proyecto:
   ...
   1. En el cuadro de diálogo, expanda el [!DNL Maven] carpeta, seleccione [!DNL Maven] proyecto y haga clic en **[!UICONTROL Siguiente]**.
...
1. Configure las variables [!DNL Java] compilador a la versión 1.5:
   ...
   1. Haga clic con el botón derecho en el `myBundle` proyecto, seleccione [!UICONTROL Propiedades].
   1. Select [!UICONTROL Compilador Java] y establezca las siguientes propiedades en 1.5:
...
1. Reemplace el código en la variable `pom.xml` con el siguiente código:
...
1. Creación del paquete `com.day.cq5.myhandler` que contiene el [!DNL Java] clases bajo `myBundle/src/main/java`:
...
1. Cree la variable [!DNL Java] class `MyHandler`:

   1. En [!DNL Eclipse], en `myBundle/src/main/java`, haga clic con el botón derecho en el `com.day.cq5.myhandler` paquete. Select [!UICONTROL Nuevo], luego [!UICONTROL Clase].
   1. En la ventana de diálogo, asigne un nombre a la variable [!DNL Java] class `MyHandler` y haga clic en [!UICONTROL Finalizar]. [!DNL Eclipse] crea y abre el archivo `MyHandler.java`.
...
1. Compile el [!DNL Java] y cree el paquete:
   ...
   1. Haga clic con el botón derecho en el `myBundle` proyecto, seleccione **[!UICONTROL Ejecutar como]**, luego **[!UICONTROL Instalación de Maven]**.
...
>[!NOTE]
>
>En sistemas que no son de Windows, la herramienta FFmpeg devuelve un error al generar representaciones para un recurso de vídeo que tiene una comilla simple (&#39;) en su nombre de archivo. Si el nombre del archivo de vídeo incluye una comilla simple, elimínelo antes de cargarlo a [!DNL Experience Manager].
...
* Transmite el resultado de nuevo a la carpeta de representación de la variable [!DNL Experience Manager] servidor.
...
El siguiente ejemplo muestra cómo configurar el paso del proceso de la línea de comandos para que cada vez que se añada un recurso con el GIF o TIFF de tipo e-MIME se añada a `/content/dam` en el [!DNL Experience Manager] servidor, se crea una imagen giratoria del original junto con tres miniaturas adicionales (140x100, 48x48 y 10x250).
...
1. Para ver si la herramienta está instalada correctamente, ejecute el siguiente comando `convert -h` en la línea de comandos.
   ...
   >[!NOTE]
   >
   >En algunas versiones de Windows, es posible que el comando convertir no se ejecute correctamente porque está en conflicto con la utilidad de conversión nativa que forma parte de [!DNL Windows] instalación. En este caso, mencione la ruta completa para la variable [!DNL ImageMagick] software utilizado para convertir archivos de imagen en miniaturas. Por ejemplo, `"C:\Program Files\ImageMagick-6.8.9-Q16\convert.exe" -define jpeg:size=319x319 ${filename} -thumbnail 319x319 cq5dam.thumbnail.319.319.png`.

1. Para ver si la herramienta se ejecuta correctamente, añada una imagen de JPG al directorio de trabajo y ejecute el comando convertir `<image-name>.jpg -flip <image-name>-flipped.jpg` en la línea de comandos. Se añade una imagen invertida al directorio. A continuación, agregue el paso del proceso de la línea de comandos al flujo de trabajo de **[!UICONTROL recursos de actualización de DAM.]**
...
1. Cambie el [!UICONTROL Argumentos] del **[!UICONTROL Representación habilitada para web]** paso a: `mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`.
...
1. Abrir el recurso **[!UICONTROL myImage.tiff]** y compruebe que se hayan creado la imagen girada y las tres miniaturas.
...
Separe los valores de la variable [!UICONTROL Argumentos de proceso] usar una coma y no iniciarla con un espacio en blanco.
...
| cmd: &lt;command> | Define el comando que se ejecuta. La sintaxis depende de la herramienta de línea de comandos. Solo se puede definir un comando. <br>Para crear el comando se pueden usar las siguientes variables:<br>`${filename}`: nombre del archivo de entrada, por ejemplo original.jpg <br> `${file}`: nombre completo de ruta del archivo de entrada, por ejemplo `/tmp/cqdam0816.tmp/original.jpg` <br> `${directory}`: directorio del archivo de entrada, por ejemplo `/tmp/cqdam0816.tmp` <br>`${basename}`: nombre del archivo de entrada sin su extensión, por ejemplo, original <br>`${extension}`: extensión del archivo de entrada, por ejemplo JPG. |

Por ejemplo, si [!DNL ImageMagick] está instalado en el disco que aloja el [!DNL Experience Manager] servidor y si crea un paso de proceso mediante [!UICONTROL CommandLineProcess] como Implementación y los siguientes valores como [!UICONTROL Argumentos de proceso]:

`mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`

a continuación, cuando se ejecute el flujo de trabajo, el paso solo se aplicará a los recursos que tengan `image/gif` o `mime:image/tiff` como `mime-types`, crea una imagen volteada del original, la convierte en JPG y crea tres miniaturas con las dimensiones: 140x100, 48x48 y 10x250.

Utilice lo siguiente [!UICONTROL Argumentos de proceso] para crear las tres miniaturas estándar con [!DNL ImageMagick]:

`mime:image/tiff,mime:image/png,mime:image/bmp,mime:image/gif,mime:image/jpeg,cmd:convert ${filename} -define jpeg:size=319x319 -thumbnail "319x319>" -background transparent -gravity center -extent 319x319 -write png:cq5dam.thumbnail.319.319.png -thumbnail "140x100>" -background transparent -gravity center -extent 140x100 -write cq5dam.thumbnail.140.100.png -thumbnail "48x48>" -background transparent -gravity center -extent 48x48 cq5dam.thumbnail.48.48.png`

Utilice lo siguiente [!UICONTROL Argumentos de proceso] para crear la representación habilitada para web mediante [!DNL ImageMagick]:

`mime:image/tiff,mime:image/png,mime:image/bmp,mime:image/gif,mime:image/jpeg,cmd:convert ${filename} -define jpeg:size=1280x1280 -thumbnail "1280x1280>" cq5dam.web.1280.1280.jpeg`

>[!NOTE]
>
>La variable [!UICONTROL CommandLineProcess] el paso solo se aplica a los recursos (nodos de tipo `dam:Asset`) o descendientes de un recurso.
