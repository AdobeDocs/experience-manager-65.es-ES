...
Se puede configurar un flujo de trabajo para que se ejecute automáticamente cuando se cargue un recurso de un tipo MIME concreto. Los pasos de procesamiento se definen en términos de una serie de [!DNL Assets] controladores de medios. [!DNL Experience Manager] proporciona algunos [controladores integrados](#default-media-handlers), y las adicionales pueden ser [desarrollado a medida](#creating-a-new-media-handler) o se define delegando el proceso a un [herramienta de línea de comandos](#command-line-based-media-handler).
...
>[!NOTE]
>
>Consulte la [Formatos de recurso admitidos](assets-formats.md) para obtener una descripción de todos los formatos admitidos por [!DNL Assets] y funciones compatibles con cada formato.
...
[!DNL Experience Manager] tiene algunos flujos de trabajo predeterminados para procesar recursos. Para verlos, abra la consola Flujo de trabajo y haga clic en **[!UICONTROL Modelos]** pestaña: los títulos del flujo de trabajo que comienzan con [!DNL Assets] son los específicos de los recursos.
...
* `com.day.cq.dam.api.handler.AssetHandler` interfaz: Esta interfaz describe el servicio que añade compatibilidad con tipos MIME específicos. Añadir un tipo MIME requiere implementar esta interfaz. La interfaz contiene métodos para importar y exportar documentos específicos, para crear miniaturas y extraer metadatos.
...
1. En Eclipse, cree `myBundle` [!DNL Maven] proyecto:
   ...
   1. En el cuadro de diálogo, expanda [!DNL Maven] carpeta, seleccione la [!DNL Maven] proyecto y haga clic en **[!UICONTROL Siguiente]**.
...
El `CommandLineProcess` process realiza las siguientes operaciones en el orden indicado:
...
1. Configure la herramienta para que una vez en la línea de comandos, pueda ejecutar `convert`.
...
A continuación, cuando se ejecute el flujo de trabajo, el paso solo se aplicará a los recursos que tengan `image/gif` o `mime:image/tiff` as `mime-types`. Crea una imagen volteada del original, la convierte en JPG y crea tres miniaturas con las dimensiones 140x100, 48x48 y 10x250.
