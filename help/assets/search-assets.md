...
# Búsqueda de recursos digitales en [!DNL Adobe Experience Manager] {#search-assets-in-aem}
...
| [Comprender la IU de búsqueda](#searchui) | [Búsqueda visual o de similitudes](#configvisualsearch) | [Comprobación de propiedades y metadatos de un recurso](#checkinfo) |
...
El uso de etiquetas inteligentes añade un `OR` para encontrar cualquiera de los términos de búsqueda como etiquetas inteligentes aplicadas. Un recurso etiquetado con `woman` o `running` Las etiquetas inteligentes también aparecen en este tipo de consultas de búsqueda. Por lo tanto, los resultados de búsqueda son una combinación de:

* Recursos con `woman` y `running` palabras clave en los metadatos (comportamiento predeterminado).

* Recursos etiquetados inteligentes con cualquiera de las palabras clave (comportamiento Etiquetas inteligentes).
...
![Escriba varias palabras clave para ver las sugerencias que se ajusten a todo](assets/search_suggestionsmanykeywords.gif)

*Imagen: escriba varias palabras clave para ver las sugerencias que se ajusten a todas.*
...
### Búsqueda de recursos digitales en [!DNL Experience Manager] aplicación de escritorio {#desktop-app}
...
### Búsqueda de recursos digitales en [!DNL Brand Portal] {#brand-portal}
...
>[!NOTE]
>
>Se ha llamado al selector de recursos [selector de recursos](https://helpx.adobe.com/experience-manager/6-2/assets/using/asset-picker.html) en versiones anteriores de [!DNL Adobe Experience Manager].
...
* Para excluir recursos específicos de los resultados de búsqueda, utilice `excludedPath` en el índice Lucene.
...
1. Entrada [!DNL Experience Manager] CRXDE, en `/oak:index/lucene` , agregue las siguientes propiedades y valores y guarde los cambios.
...
1. Para crear el índice Lucene, en CRXDE, en `/oak:index/damAssetLucene/indexRules/dam:Asset/properties`, cree un nodo llamado `imageFeatures` de tipo `nt-unstructured`. Entrada `imageFeatures` nodo,
...
1. En CRXDE, en `/oak-index/damAssetLucene` , establezca el `reindex` propiedad a `true`. Guarde los cambios.
1. (Opcional) Si ha personalizado el formulario de búsqueda, copie el `/libs/settings/dam/search/facets/assets/jcr%3Acontent/items/similaritysearch` nodo a `/conf/global/settings/dam/search/facets/assets/jcr:content/items`. Guarde los cambios.

Para obtener información relacionada, consulte [explicación de las etiquetas inteligentes en Experience Manager](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/metadata/image-smart-tags.html) y [cómo administrar etiquetas inteligentes](/help/assets/enhanced-smart-tags.md).
...
Puede configurar [!DNL Experience Manager] para extraer el texto de los recursos cuando los usuarios cargan recursos, como archivos de PSD o PDF. [!DNL Experience Manager] indexa el texto extraído y ayuda a los usuarios a buscar estos recursos en función del texto extraído. Consulte [cargar recursos](/help/assets/manage-assets.md#uploading-assets).

Si la extracción de texto consume demasiados recursos para su implementación, considere lo siguiente [deshabilitar la extracción de texto](https://helpx.adobe.com/experience-manager/kb/Disable-binary-text-extraction-to-optimize-Lucene-indexing-AEM.html).
...
En la vista de lista, puede ordenar los resultados de búsqueda del mismo modo que puede ordenar los recursos de cualquier carpeta. La ordenación funciona en estas columnas: Nombre, Título, Estado, Dimension, Tamaño, Clasificación, Uso, Creado, Modificado, Publicado, Flujo de trabajo y Extraído.
