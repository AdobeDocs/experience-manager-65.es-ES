---
title: Notas de la versión [!DNL Adobe Experience Manager] 6.5.13.0
...
[!DNL Experience Manager] 6.5.13.0 incluye nuevas características, mejoras clave solicitadas por el cliente y mejoras de rendimiento, estabilidad y seguridad, que se publicaron tras la publicación inicial de la versión 6.5 en abril de 2019. [Instalar este Service Pack](#install) en [!DNL Experience Manager] 6.5.

Las principales características y mejoras introducidas en [!DNL Adobe Experience Manager] 6.5.13.0 son:
...
* Cuando localiza un formulario adaptable y realiza incluso un pequeño cambio en el texto del idioma base, la traducción completa no aparece para todos los demás idiomas. El problema se corrigió en [!DNL Experience Manager] 6.5.13.0. (NPR-37189)
...
* Mejoras de accesibilidad para Forms:
   ...
   * Se ha agregado compatibilidad para que los lectores de pantalla dejen de navegar por HTML Workspace hasta que se abra un cuadro de diálogo. (NPR-37134)

Se introdujeron las siguientes correcciones de errores, características clave y mejoras en [!DNL Experience Manager] 6.5.13.0:
...
* Al agregar una regla a un campo desplegable en **[!UICONTROL Esquema de metadatos]**, la lista **[!UICONTROL Dependiente de]** no refleja la etiqueta de campo de la lista desplegable. (ASSETS-9442)
* Metadatos de Assets desactivados en la lista desplegable que no retienen valores. (ASSETS-8918)
* Al ver el recurso mediante la opción **[!UICONTROL Más detalles]** en la vista **[!UICONTROL Columna]**, se muestran anotaciones incorrectas. (ASSETS-8851)
...
* Después de agregar un componente de Dynamic Media en un sitio y después de publicar la página, el recurso de Dynamic Media recién agregado no es visible en la página publicada ni en la página de vista previa. Este problema se producía tanto para tipos de recursos de imagen como de vídeo. (ASSETS-9467)
...
* Los usuarios experimentan texto superpuesto en el Forms adaptable después de aplicar el paquete de servicio 11 de [!DNL Experience Manager] 6.5. (NPR-38376, CQ-4342472)
...
* Los usuarios han cerrado la sesión aleatoriamente de [!DNL Experience Manager] instancias de publicación y no se puede enviar un formulario adaptable. El problema aparece en [!DNL Experience Manager] instancias que utilizan Dispatcher. (NPR-38285)
...
* [!DNL Experience Manager] 6.5 Forms API obsoleta relacionada con java.acl.Group y los siguientes mensajes de error aparecen en el archivo error.log:
...
* El componente de AEM Forms incrustado en la página SITES utiliza el atributo de anchura que tiene un valor en % y no es válido según la validación de HTML del W3C. Los usuarios encuentran un error de análisis incorrecto durante la validación de HTML. (NPR-38124)
...
* Un formulario con la propiedad Prepare data process establecida no se puede procesar en HTML Workspace. (CQ-4343294)
...
* No se puede importar un archivo de Microsoft® Word en Forms Designer. El usuario encuentra el error `Word (version XP or onwards) could not be found on the machine`. (CQ-4342146)
* El usuario no puede establecer una hora personalizada en el planificador de purga de trabajos. (CQ-4339192)
* El usuario no puede actualizar ninguna configuración en la interfaz de usuario de administración de extremos y encontró el error ` Uncaught ReferenceError: updateEndpoint_required is not defined`. (CQ-4331523)
...
* Habilite ES6 para `/libs/granite/configurations/clientlibs/confbrowser`. (NPR-38300)
...
* [!DNL Experience Manager] Platform 6.5: correcciones de nomenclatura de configuración regional china. (CQ-4308978)
* Cambie a Launch para Omega Instrumentation. (NPR-38377)
* [!DNL Experience Manager] Platform 6.5: correcciones chinas de nomenclatura de configuración regional. (CQ-4308978)
...
* El uso de **[!UICONTROL Edit]** de los resultados de búsqueda podría provocar un error de `Not Found`. (NPR-37810)
...
* Los modelos de interfaz de usuario de Context Hub no se representan correctamente sin una actualización de página completa. (NPR-38212)
...
* Agregar la propiedad `cq:isTranslationLaunch` en los lanzamientos creados por el proyecto de traducción. (NPR-38224)
...
1. Descargue el Service Pack de [Distribución de software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.13.0.zip).
...
>[!NOTE]
>
>Omitir si no utiliza [!DNL Experience Manager] Forms. Las correcciones de [!DNL Experience Manager] Forms se entregan mediante un paquete de complementos independiente una semana después del lanzamiento programado del paquete de servicio [!DNL Experience Manager].
