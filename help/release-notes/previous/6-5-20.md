...
* Al ejecutar una consulta GraphQL para [filtrar resultados](/help/sites-developing/headless/graphql-api/graphql-api-content-fragments.md#filtering) mediante variables opcionales, si se proporciona un valor específico de **no** para la variable opcional, entonces la variable se omitirá en la evaluación del filtro. (SITES-17051)
...
* Con la actualización de la biblioteca `org.json`, se produjo un cambio en la forma en que se deserializaron los números decimales. Antes se convertían &quot;por defecto&quot; en Dobles y ahora en BigDecimals. En su lugar, los valores de las propiedades de metadatos, almacenados mediante la API de REST, deben convertirse a Double a partir de BigDecimal. (SITES-16857)

#### Servidor principal{#sites-core-backend-6520}

* Cuando se utiliza la publicación rápida de un fragmento de contenido, continúa cargándose y no se publica. Es decir, Publicación rápida no funciona para fragmentos de contenido después de una actualización de Service Pack de AEM 6.5.7 a AEM 6.5.17. Cuando el usuario intentó la publicación administrada, funcionó. Sin embargo, cuando intentaron Publicación rápida, no se estaba publicando. Específicamente, `com.day.cq.wcm.core.impl.reference.ActivationReferenceSearchBuilder` causó que el sistema se reprodujera. (SITES-17311)
...
* La instalación del paquete de componentes principales de CIF en AEM provoca que cambie el valor `:type` de los componentes existentes. El cambio significa que ya no se representan en las páginas a las que se han agregado. (SITES-17601)
...
* Se produce una excepción &quot;`Is not modifiable`&quot; al desplegar el componente. Específicamente, se experimenta una excepción `org.apache.sling.servlets.post.impl.operations.ModifyOperation` durante el procesamiento de la respuesta. (SITES-18809)
...
* Una vez que PDF se haya movido a otra ubicación, la opción **[!UICONTROL Ver página]** desaparecerá. (ASSETS-30538)
...
* Cuando se carga un recurso en AEM, se activa el flujo de trabajo `Update_asset`. Sin embargo, el flujo de trabajo nunca termina. El flujo de trabajo solo finaliza hasta el paso de carga del producto. El siguiente paso es la carga por lotes de Scene7, pero ese proceso no se está extrayendo en AEM. (ASSETS-30443)
...
* Se corrigió el estado de `scene7SmartCropProcessingStatus`. Los metadatos de vídeo de recorte inteligente se utilizaban para mostrar errores incluso si se habían realizado correctamente. (ASSETS-31255)
...
* Cuando un usuario intenta integrar AEM Forms en una plataforma de correo con una dirección URL publicada por AEM, AEM Forms no agrega `method=post` al procesar la página. Este problema ocurre aunque `POST` esté establecido en la acción de envío con la dirección URL. Esto hace que la plataforma de correo no reconozca este formulario. (FORMS-12614)
* Cuando un usuario selecciona el campo de fecha que tiene un patrón de visualización en AEM Form Service Pack 6.5.18.0, no puede seleccionar la fecha actual mediante el teclado. (FORMS-12736)
* En AEM Forms Service Pack 6.5.17.0 y Service Pack 6.5.18.0, cuando un usuario cambia entre meses en el widget de calendario, el componente selector de fecha muestra una fila adicional. (FORMS-11869)
...
* Cuando un usuario agrega caracteres a un formulario adaptable en un dispositivo Android™, puede escribir más del número máximo de caracteres definido en el campo Texto al enfocar hacia fuera, en dispositivos Android™. Sin embargo, funciona cuando un usuario selecciona el tipo de entrada HTML5. (FORMS-12748)
...
* Cuando un usuario actualiza a AEM Form Service Pack 6.5.19.0, después de actualizar un nuevo idioma al diccionario existente, no se combina con las filas &quot;guideContainer&quot; para agregar una configuración regional a un formulario. (FORMS-12947)
...
* Cuando el modo FIPS está habilitado en el Service Pack de AEM Forms 6.5.18.0, al crear un usuario en el DOM predeterminado se produce el siguiente error (FORMS-11857):
...
* Cuando un usuario del paquete de servicio de AEM Forms 6.5.18.0 envía un formulario adaptable en el flujo de trabajo JEE, el archivo adjunto del formulario adaptable no se envía al proceso JEE, lo que provoca un error en la aplicación. (FORMS-12232, FORMS-12228)

* Cuando un usuario convierte PDF a PDF/A-2b o PDF/A-3B, no se convierte, el error se muestra de la siguiente manera: (FORMS-12790)
...
* Cuando un usuario selecciona &quot;Microsoft® SharePoint&quot; mientras ejecuta el administrador de configuración en AEM Forms 6.5.19.1 con la instalación de JBoss® Turnkey, la instalación de LiveCycle JBoss® EAR falla y muestra el siguiente error: (FORMS-12463)
...
* Cuando un usuario actualiza al paquete de servicio de AEM Forms 6.5.18.0, debido a la falta de administración de excepciones, los XDP pasados a través del servicio de salida con la opción PDF etiquetada como habilitada dan error. (LC-3921757)
...
* Cuando un usuario crea un XDP en AEM Forms Designer, en la salida de PDF cuando navega por las etiquetas del formulario, el objeto de fondo también se etiqueta. (LC-3921687)
...
* La etiqueta Aria &quot;close&quot; no está localizada en **[!UICONTROL Assets]** > **[!UICONTROL Archivos]**, seleccione una carpeta y, en la barra de herramientas, seleccione **[!UICONTROL Propiedades]** > **[!UICONTROL Permisos]** > pestaña Nombre de miembro. (NPR-41705)
* Hay una información de objeto truncada para el campo **[!UICONTROL Contraseña del almacén de claves]** en la página Configuración SSL para las configuraciones regionales ENG, FRA, KOR, DEU y PTB. (NPR-41367)

#### Plataforma{#foundation-platform-6520}

* Problema con la integración de Campaign con AEM debido a que el servlet /api no devuelve el esquema correcto en el json href. El motivo era que AEM no recibía el encabezado X-Forward-Proto, lo que obligaba a la solicitud a responder con un esquema HTTP en lugar de HTTPS. Como tal, se debe añadir la capacidad de alternar la selección de esquemas en función de una configuración OSGI. (GRANITE-48454)

#### Sling{#foundation-sling-6520}

* El paquete 1.4.2 de `org.apache.sling.resourceMerger` genera una excepción de AEM 6.5, Service Pack 17 y posterior. La fusión de recursos de Sling 1.4.4 debe incluirse en el paquete de servicio 20. (NPR-41630)

#### Traducción{#foundation-translation-6520}

* Después de la implementación del paquete de servicio 18 de AEM 6.5, se produjo un problema con la pestaña Filtros en el Editor de reglas de traducción. Cuando se selecciona un Contexto, al hacer clic en Editar > Guardar, la próxima vez que abra el mismo Contexto aparecerá un carácter de comillas dobles como HTML. Básicamente, las reglas de traducción no se guardaban correctamente. (NPR-41624)
...
* El error de la consola se experimenta después de una actualización a AEM 6.5, Service Pack 18. El error se encuentra en el archivo `coralUI3.js` y se produce al seleccionar cualquier lista desplegable en AEM. Específicamente, sucede con un evento `onOverlayToggle`. Se muestra el error `Uncaught TypeError: Cannot read properties of null (reading 'innerText')`. (NPR-41467)
* En AEM, **[!UICONTROL Herramientas]** > **[!UICONTROL General]** > **[!UICONTROL Etiquetado]** > **[!UICONTROL Crear]** > **[!UICONTROL Crear etiqueta]**, al escribir caracteres no latinos en el campo **Título**, el campo **Nombre** se rellenará con solo el carácter de guión (`-`). (NPR-41623)
...
* La descarga del Service Pack está disponible en Adobe [Distribución de software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq650/servicepack/aem-service-pkg-6.5.20.0.zip).
* En una implementación con MongoDB y varias instancias, instale [!DNL Experience Manager] 6.5.20.0 en una de las instancias de creación mediante el Administrador de paquetes.<!-- UPDATE FOR EACH NEW RELEASE -->
...
1. El paquete OSGi `org.apache.jackrabbit.oak-core` es de la versión 1.22.18 o posterior (utilice la consola web: `/system/console/bundles`). <!-- OAK Oak oak VERSION -MAY- NEED TO BE UPDATED FOR EACH NEW RELEASE -->
