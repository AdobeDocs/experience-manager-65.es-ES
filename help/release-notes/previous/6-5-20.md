...
* Cuando se utiliza el Quick Publish de un fragmento de contenido, continúa cargándose y no se publica. Es decir, Quick Publish AEM AEM no funciona para los fragmentos de contenido después de una actualización de Service Pack de la versión 6.5.7 a la versión 6.5.17 de la versión 6.5.7 de la aplicación de. Cuando el usuario intentó la publicación administrada, funcionó. Sin embargo, cuando intentaron Quick Publish, no se estaba publicando. Específicamente, `com.day.cq.wcm.core.impl.reference.ActivationReferenceSearchBuilder` causó que el sistema se reprodujera. (SITES-17311)
...
* CIF AEM La instalación del paquete de componentes principales en la hace que cambie el valor `:type` de los componentes existentes. El cambio significa que ya no se representan en las páginas a las que se han agregado. (SITES-17601)
...
* Cuando un PDF se mueve a otra ubicación, la opción **[!UICONTROL Ver página]** desaparece. (ASSETS-30538)
...
* Cuando un usuario crea un XDP en AEM Forms Designer, en la salida del PDF, la etiqueta del formulario de botón no se anida en la etiqueta de párrafo principal (p-tag). (LC-3921719)

* Cuando un usuario crea un XDP en AEM Forms Designer, en la salida del PDF cuando este navega por las etiquetas del formulario, el objeto de fondo también se etiqueta. (LC-3921687)
...
* AEM El paquete 1.4.2 de `org.apache.sling.resourceMerger` genera una excepción con respecto a la versión 6.5, el paquete de servicio 17 y posteriores de la versión de. La fusión de recursos de Sling 1.4.4 debe incluirse en el paquete de servicio 20. (NPR-41630)
...
* AEM Problemas relacionados con las traducciones de fragmentos de contenido, en las que las cadenas traducidas se envían de vuelta del proveedor de traducción a los recursos, pero se quedan en el nivel `/content/projects` y no actualizan los fragmentos de contenido. (NPR-41516)
...
* AEM El error de la consola se experimenta después de una actualización al paquete de servicio 18 de la versión 6.5 de la. AEM El error se encuentra en el archivo `coralUI3.js` y se produce al seleccionar cualquier lista desplegable en la lista de elementos de la lista de elementos de la lista de elementos de la lista de. Específicamente, sucede con un evento `onOverlayToggle`. Se muestra el error `Uncaught TypeError: Cannot read properties of null (reading 'innerText')`. (NPR-41467)
* AEM En la, **[!UICONTROL Herramientas]** > **[!UICONTROL General]** > **[!UICONTROL Etiquetado]** > **[!UICONTROL Crear]** > **[!UICONTROL Crear etiqueta]**, al escribir caracteres no latinos en el campo **Título**, el campo **Nombre** se rellenará con solo el carácter de guión ( `-` ). (NPR-41623)
...
>[!IMPORTANT]
>
> El Adobe no recomienda quitar ni desinstalar el paquete [!DNL Experience Manager] 6.5.20.0. Como tal, antes de instalar el paquete, debe crear una copia de seguridad de `crx-repository` en caso de que deba revertirla. <!-- UPDATE FOR EACH NEW RELEASE -->
...
* [Lista de paquetes OSGi incluidos en el Experience Manager 6.5.20.0](/help/release-notes/assets/65200-bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Lista de paquetes de contenido incluidos en el Experience Manager 6.5.20.0](/help/release-notes/assets/65200-packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->
