...
A continuación se muestra la lista de correcciones que se proporcionan en la versión [!DNL Experience Manager] 6.5.7.0.

### [!DNL Sites] {#sites-6570}

* Cuando se abre la opción [!UICONTROL Intervalo] de tiempo para una página, se mantiene abierta la opción de carril lateral de la línea de tiempo y se desplaza a la consola [!UICONTROL Sitios] , se produce el `Failed to Load` error (NPR-34951).

* La opción [!UICONTROL Timewrap] no muestra imágenes para el intervalo de fecha y hora seleccionado (NPR-34951).

* Cuando un filtro llama `getHeader()` desde una página que contiene un fragmento de contenido, se produce el `java.lang.AbstractMethodError` error (NPR-34942).
...
* Cuando se establece un valor numérico para la propiedad de etiqueta de `String` tipo de un componente, se puede eliminar el componente y deshacer la operación de eliminación. Sin embargo, después de deshacer la operación de eliminación, la propiedad label cambia de `String` a `Long` (NPR-34739).
...
* Cuando se crean, publican y mueven nuevos recursos a una nueva ubicación, el flujo de trabajo se crea y se anula el estado del `Request to complete move operation` flujo de trabajo. Al cargar un nuevo recurso y ejecutar una `move` operación, se crea el `Request to complete move operation` flujo de trabajo en estado pendiente (NPR-34543).

* Cuando se exporta un fragmento de experiencia de [!DNL Experience Manager] 6.5.2 a [!DNL Target] Standard, la llamada de API falla porque la propiedad del espacio de trabajo no está disponible para [!DNL Target] Standard (NPR-34557).
...
* Al agregar algunos estilos al texto, se agrega una `<div>` etiqueta al texto y el estilo ya no se puede aplicar al texto (NPR-34531).
...
* Cuando un usuario selecciona la opción de cuadro de diálogo [!UICONTROL Abrir selección] , el enfoque del teclado pasa al control oculto presente en la página (CQ-4307779, CQ-4293601).
...
* Cuando un usuario selecciona la `Enter` tecla en la opción [!UICONTROL Seleccionar todo] , el enfoque del teclado no se mueve a la opción [!UICONTROL Crear control] (CQ-4293599).

* Al seleccionar la `Esc` clave, el enfoque no se restaura al control principal (CQ-4293593, CQ-4293590).

* Se ha mejorado la compatibilidad con WCAG para [!DNL Sites] la interfaz de usuario y los componentes principales (CQ-4293448).

* [!UICONTROL Las funciones Zoom] y [!UICONTROL Escala] están desactivadas para la [!DNL Sites Editor] página (CQ-4282353).
...
* Los usuarios del teclado no pueden cambiar el enfoque de la ficha en un orden adecuado después de seleccionar la sección [!UICONTROL Acción] en una [!DNL AEM Sites] página (CQ-4307786).
...
* El enfoque de tabulación no cambia en el orden adecuado al seleccionar una fecha en el menú Calendario de la ficha [!UICONTROL Recursos] de una [!DNL Sites] página (CQ-4293600).
...
* Los usuarios del teclado no pueden volver a cambiar el enfoque de tabulación a Más opciones en la sección [!UICONTROL Acciones] después de ver las opciones disponibles y pulsar la `Esc` tecla (CQ-4293592).
...
* En el cuadro de diálogo [!UICONTROL Abrir selección] disponible en la ficha [!UICONTROL Vínculo y acciones] , el enfoque de tabulación cambia a elementos ocultos en la página después de la opción [!UICONTROL Cancelar] (CQ-4293579).

* Cuando los usuarios del teclado editan una imagen, acceden a la opción [!UICONTROL Finalizar] y presionan la tecla Intro, los lectores de pantalla no anuncian la finalización (CQ-4282351).
...
* Los usuarios sin privilegios de administrador pueden editar recursos caducados mediante la aplicación de escritorio de [Experience Manager] . (NPR-34993).
...
* Cuando se utiliza [!DNL Adobe Asset Link] con [!DNL Adobe InDesign], los resultados de búsqueda no contienen carpetas y colecciones, sino que solo contienen recursos (NPR-34699, CQ-4303666).
...
* Al editar las propiedades de varios recursos de forma masiva, al seleccionar la opción [!UICONTROL Guardar] se cierra la vista del editor masivo y se redirige a la [!DNL Assets] página principal. Este comportamiento es el mismo que el de la opción [!UICONTROL Guardar y cerrar] y no se espera (NPR-34546).
...
* Las siguientes mejoras se realizan para la accesibilidad en [!DNL Dynamic Media] (CQ-4290306). Para obtener más información, consulte Funciones [de accesibilidad en [!DNL Dynamic Media]](/help/assets/accessibility-dm.md).
   ...
   * Al navegar con `Tab` clave, el enfoque se desplaza a los elementos de interfaz de usuario adecuados en el flujo de trabajo para crear un vídeo interactivo (CQ-4290621, CQ-4290620, CQ-4290619).
...
* El orden del índice z es incorrecto cuando se intenta previsualización de la salida después de agregar una zona interactiva a una [!DNL Dynamic Media] imagen o después de editar un [!DNL Dynamic Media] vídeo o una imagen [!DNL Experience Fragment] (CQ-4307267).

* [!DNL Dynamic Media] la sincronización falla cuando se vuelven a procesar conjuntos de medios mixtos (CQ-4307184).

* Si un recurso se mueve a una carpeta en la que [!DNL Dynamic Media] está configurada la sincronización automática, el recurso no se sincroniza (CQ-4307122).
...
* Al cancelar la publicación de un archivo de vídeo de [!DNL Experience Manager] no se cancelará la publicación del conjunto de vídeos adaptable a partir de una implementación de Scene7 configurada (CQ-4304405).

* Añadir un recurso de imagen panorámica en un componente de medios panorámico y actualizar la página resulta en un `Uncaught ReferenceError: $ is not defined` error (CQ-4302810).

* En el Editor de ajustes preestablecidos de visor, al editar el ajuste preestablecido [!UICONTROL PanoramicImage/PanoramicImage_VR] , en el `PanoramicView` componente no está disponible la etiqueta del `PANORAMICVIEW_AUTOROTATE` modificador (CQ-4302443).
...
* Al intentar vincular un fragmento de experiencia para una zona interactiva determinada en el componente Vídeo [!UICONTROL interactivo/Imágenes] , no muestra la ruta de fragmento de experiencia seleccionada. En su lugar, devuelve un valor vacío desde el campo de ruta (NPR-35146, CQ-4298136).
...
* Se `NullPointerException` devuelve un valor cuando se inician tareas de mantenimiento diarias o semanales desde la interfaz de usuario (NPR-34953).
* El validador de W3C informa de advertencias para archivos JavaScript de la biblioteca cliente compatibles (NPR-34898).
* La `AudienceOmniSearchHandler` función utiliza un índice obsoleto (NPR-34870).
* Al cerrar sesión en el Experience Manager no se borran las cookies (NPR-34743).
* La `findByTitle` función de la `TagManager` API no funciona si el nombre de la etiqueta contiene un carácter especial (NPR-34357).
* Error en el proceso de importación del paquete de sincronización de usuarios (NPR-34399).
* Se añadió la compatibilidad `ariaLabel` y las `ariaLabelledby` propiedades del `Coral.Masonry` componente (GRANITE-29962).
...
* El campo [!UICONTROL Vínculo a] de las propiedades del componente muestra sugerencias de autocompletar que no coinciden con la cadena especificada (NPR-34865).
...
* Error al editar una [!DNL Adobe Launch] configuración existente (NPR-35045).
* No se puede exportar [!DNL Experience Fragments] a [!DNL Adobe Target] si se utiliza la configuración IMS y el [!DNL Adobe Target Standard] entorno (NPR-34555).
* La opción [!UICONTROL Crear] aparece en la página [!UICONTROL Audiencias] al desplazarse de una carpeta a la página [!UICONTROL Audiencias] (NPR-35151).
...
* La interfaz de usuario no responde al seleccionar los recursos en la Vista [!UICONTROL de] Lista (NPR-34728).

### [!DNL Forms] {#forms-6570}

>[!NOTE]
>
>[!DNL Experience Manager Forms] libera los paquetes de complementos una semana después de la fecha de lanzamiento programada [!DNL Experience Manager] de Service Pack.
...
1. Antes de realizar la instalación, realice una instantánea o una copia de seguridad nueva de la instancia de [Experience Manager] .
...
1. La página de información del producto (`/system/console/productinfo`) muestra la cadena de versión actualizada `Adobe Experience Manager (6.5.7.0)` en Productos instalados.
...
UberJar para Experience Manager 6.5.7.0 está disponible en el repositorio [de](https://repo.maven.apache.org/maven2/com/adobe/aem/uber-jar/6.5.7/)Maven Central.
...
>[!NOTE]
>
>UberJar y los otros artefactos relacionados están disponibles en el repositorio de Maven Central en lugar del repositorio de Adobe Public Maven (`repo.adobe.com`). Se cambia el nombre del archivo principal de UberJar a `uber-jar-<version>.jar`. Por lo tanto, no hay `classifier`, con `apis` como valor, para la `dependency` etiqueta.

## Funciones en desuso {#removed-deprecated-features}

A continuación se muestra una lista de funciones y funciones que se han marcado como obsoletas con [!DNL Experience Manager] 6.5.7.0. Las funciones se marcan como obsoletas inicialmente y posteriormente se eliminan en una versión futura. Normalmente se proporciona una opción alternativa.
...
* Durante la instalación de Experience Manager 6.5.7.0, omita los siguientes errores en el `error.log` archivo:
...
* Si está actualizando su [!DNL Experience Manager] instancia de la versión 6.5 a la versión 6.5.7.0, puede vista `RRD4JReporter` de excepciones en el `error.log` archivo. Reinicie la instancia para resolver el problema.
...
* Póngase en contacto con el servicio de atención al cliente de Adobe si se producen problemas al editar y crear reglas en cascada en el Esquema de metadatos de [!UICONTROL carpeta Editor] Forms y el Editor [!UICONTROL de Forms de] metadatos mediante el cuadro de diálogo [!UICONTROL Definir regla] . Las reglas que ya se han creado y guardado funcionan correctamente.
...
* Consulte [cómo comunicarse con el servicio de asistencia](https://experienceleague.adobe.com/docs/customer-one/using/home.html)al cliente.
