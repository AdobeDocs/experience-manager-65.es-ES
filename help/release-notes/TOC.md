---
cloud: experience-cloud
product: experience manager
audience: end-user
user-guide-title: Notas de la versión de Adobe Experience Manager 6.5
breadcrumb-title: Notas de la versión
user-guide-description: Obtenga información sobre las nuevas funciones más recientes de AEM 6.5 en las notas de la versión.
translation-type: tm+mt
source-git-commit: 90ab21c4a25fa1a2dcdf04af61fd30982cae7aae
workflow-type: tm+mt
source-wordcount: '75'
ht-degree: 29%

---


# Adobe Experience Manager 6.5 Release Notes {#release-notes}

+ [Descripción general de AEM 6.5](home.md)
+ [Notas de revisión generales](release-notes.md)
+ Service Pack Release Notes {#service-pack}
   + [Novedades](new-features-latest-service-pack.md)
   + [Notas de la versión de Service Pack 6](sp-release-notes.md)
   + [Notas de la versión anteriores de Service Pack](previous-hotfixes-featurepacks.md)
+ [Notas de la versión de AEM Sites](sites.md)
+ [Notas de la versión de Activos AEM](assets.md)
+ [Instalador de parches de AEM Forms JEE](jee-patch-installer-65.md)
+ [Notas de versión de AEM Communities](communities-release-notes.md)
+ [Funciones en desuso y eliminadas](deprecated-removed-features.md)
+ [Problemas conocidos](known-issues.md)
+ [Base AEM y repositorio](wcm-platform.md)
+ [Estado de la función de IU táctil](touch-ui-features-status.md)