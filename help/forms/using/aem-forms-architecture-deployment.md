...
description: Detalles de arquitectura para AEM Forms y topologías recomendadas para clientes nuevos y existentes de AEM y clientes que actualizan de LiveCycle ES4 a AEM Forms.
seo-description: Detalles de arquitectura para AEM Forms y topologías recomendadas para clientes nuevos y existentes de AEM y clientes que actualizan de LiveCycle ES4 a AEM Forms.
...
**AEM Forms en OSGi:** Un AEM Forms en el entorno OSGi es un AEM Author estándar o un AEM Publish con el paquete de AEM Forms implementado en él. Puede ejecutar AEM Forms en OSGi en un [único entorno de servidor, granja de servidores y configuraciones](/help/sites-deploying/recommended-deploys.md)agrupadas. La configuración de clúster solo está disponible para instancias de AEM Author.

**AEM Forms en JEE:** AEM Forms en JEE es un servidor de AEM Forms que se ejecuta en la pila JEE. Cuenta con AEM Author con paquetes de complementos de AEM Forms y funciones JEE de AEM Forms adicionales que se implementan en forma conjunta en una sola pila JEE que se ejecuta en un servidor de aplicaciones. Puede ejecutar AEM Forms en JEE en configuraciones de un solo servidor y agrupadas. Los AEM Forms en JEE solo son necesarios para ejecutar la seguridad de documento, la administración de procesos y para los clientes de LiveCycle que actualicen a AEM Forms. Estos son algunos escenarios adicionales para usar AEM Forms en JEE:

* **Compatibilidad con el espacio de trabajo HTML (para clientes que utilizan el espacio de trabajo HTML):** Los AEM Forms en JEE activan el inicio de sesión único con instancias de procesamiento, sirven determinados recursos procesados en instancias de procesamiento y gestionan el envío de formularios procesados en el espacio de trabajo HTML.
* **Procesamiento** avanzado de datos de comunicaciones interactivas/formularios: Los AEM Forms en JEE pueden utilizarse para procesar datos de comunicación interactivos/formularios adicionales (y guardar los resultados en un almacén de datos adecuado) en casos de uso complejos donde se requieren capacidades avanzadas de administración de procesos.

Los AEM Forms en JEE también incluyen los siguientes servicios de soporte para los componentes de AEM:

* **Administración de usuarios integrada:** Permite que los usuarios de AEM Forms en JEE sean reconocidos como formularios AEM en usuarios OSGi y ayuda a habilitar SSO para usuarios OSGi y JEE. Esto es necesario para situaciones en las que se requiere un inicio de sesión único entre formularios AEM en OSGi y AEM Forms en JEE (por ejemplo, espacio de trabajo HTML).
* **Alojamiento de recursos:** Los AEM Forms de JEE pueden servir recursos (por ejemplo, formularios HTML5) procesados en AEM Forms de OSGi.

Los AEM Forms que crean la interfaz de usuario no admiten la creación de Documentos de registro (DOR), PDF forms y formularios HTML5. Estos recursos se diseñan con la aplicación independiente de Designer Forms y se cargan individualmente en el Administrador de AEM Forms. Como alternativa, para AEM Forms en JEE, los formularios pueden diseñarse como recursos de aplicaciones (en AEM Forms Workbench) e implementarse en AEM Forms en el servidor JEE.

Los AEM Forms en OSGi y los AEM Forms en JEE tienen funciones de flujo de trabajo. Puede crear e implementar rápidamente flujos de trabajo básicos para varias tareas en los formularios de AEM en OSGi, sin tener que instalar la capacidad de administración de procesos completa de los AEM Forms en JEE. Existen algunas diferencias en las [características del flujo de trabajo centrado en el formulario en los AEM Forms de OSGi y la capacidad de administración de procesos de los AEM Forms en JEE](capabilities-osgi-jee-workflows.md). El desarrollo y la administración de flujos de trabajo centrados en formularios para AEM Forms en OSGi utiliza las funciones conocidas de AEM Workflow y AEM Inbox.

## Terminología {#terminologies}

La siguiente imagen muestra varias configuraciones de servidor de AEM Form y sus componentes que se utilizan en una implementación de AEM Forms típica:

![aem_forms_-_recommendations_topología](assets/aem_forms_-_recommendedtopology.png)

**Autor:** Una instancia de autor es un servidor AEM Forms que se ejecuta en el modo de ejecución Autor estándar. Pueden ser AEM Forms en JEE o AEM Forms en el entorno OSGi. Está destinado a usuarios internos, formularios y diseñadores de comunicaciones interactivos, así como a desarrolladores. Activa las siguientes funcionalidades:
...
**Procesamiento:** Instancia de AEM Forms que se ejecutan en modo de ejecución de Autor sin usuarios asignados al grupo de administrador de formularios. Puede implementar AEM Forms en JEE o AEM Forms en OSGi como una instancia de procesamiento. Los usuarios no están asignados para garantizar que las actividades de creación y administración de formularios no se realicen en la instancia de procesamiento y solo se produzcan en la instancia de autor. Una instancia de procesamiento activa las siguientes funcionalidades:

* **Procesamiento de datos de formulario sin procesar procedentes de una instancia de Publish:** Esto se logra principalmente en una instancia de procesamiento mediante flujos de trabajo AEM que se activan cuando llegan los datos. Los flujos de trabajo pueden utilizar el paso del Modelo de datos de formulario que se proporciona de forma predeterminada para archivar los datos o el documento en un almacén de datos adecuado.
...
* **Almacenamiento y postprocesamiento de los datos de correspondencia que llegan desde una instancia de Publish:** Los flujos de trabajo de AEM realizan el postprocesamiento opcional de las definiciones de letras correspondientes. Estos flujos de trabajo pueden guardar los datos procesados finales en un almacén de datos externo adecuado.
...
AEM Forms clientes que planean utilizar funciones de administración de procesos de AEM Forms, por ejemplo, HTML Workspace puede tener una topología similar a la que se muestra a continuación. Los AEM Forms del servidor JEE pueden estar en un único servidor o configuración de clúster.

Si está realizando la actualización desde LiveCycle ES4, esta topología refleja de cerca lo que ya tiene en LiveCycle, excepto por la adición de AEM Author integrado a AEM Forms en JEE. Además, no hay cambios en los requisitos de clustering para los clientes que realizan una actualización. Si estaba utilizando AEM Forms en un entorno agrupado, puede continuar con el mismo en AEM 6.5 Forms. Para una nueva instalación de AEM Forms de JEE para utilizar HTML Workspace, la ejecución de la instancia de creación de AEM integrada en el entorno JEE es un requisito adicional.
...
Los clientes AEM Forms que planean utilizar las capacidades de captura de datos de AEM Forms, por ejemplo, formularios adaptables, formularios HTML5 y PDF forms, pueden tener una topología similar a la que se muestra a continuación. Esta topología también se recomienda para utilizar las capacidades de comunicación interactiva de los AEM Forms.
...
* El uso de HTML Workspace y la aplicación de AEM Forms requiere un autor o una instancia de procesamiento de AEM. Puede utilizar la instancia de creación de AEM integrada en AEM Forms del servidor JEE en lugar de configurar un servidor de creación AEM externo adicional.
* Solo se requiere una instancia de AEM Author o de procesamiento para flujos de trabajo centrados en Forms en OSGi, formularios adaptables, portal de formularios y comunicación interactiva.
...
* Los formularios AEM de la instancia OSGi integrados en AEM Forms del servidor JEE también pueden ejecutar flujos de trabajo centrados en formularios en OSGi y carpetas vigiladas.
...
Los clientes AEM Forms que planean utilizar las capacidades de captura de datos de AEM Forms, por ejemplo, formularios adaptables, formularios HTML5 y PDF forms, pueden tener una topología similar a la que se muestra a continuación. Esta topología también se recomienda para utilizar comunicaciones interactivas y Flujos de trabajo centrados en formularios en la capacidad OSGi, por ejemplo, para utilizar la bandeja de entrada de AEM y la aplicación de AEM Forms para flujos de trabajo de procesos empresariales.
