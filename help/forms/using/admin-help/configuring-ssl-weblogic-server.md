...
<table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>Utilice la misma contraseña que utilizó para la opción <code>-storepass</code>. La contraseña de la clave debe tener al menos seis caracteres.</p></td>
  </tr>
  <tr>
   <td><p>-dname</p></td>
...
1. Desde el símbolo del sistema, vaya a *[appserverdomain]*/adobe/*[nombre del servidor]*.
1. Introduzca el siguiente comando:
   ...
   El archivo del almacén de claves de identidad personalizada llamado &quot;ads-credentials.jks&quot; se creará en el directorio [appserverdomain]/adobe/[nombre del servidor].

1. Extraiga el certificado del almacén de claves ads-credentials introduciendo el siguiente comando:

   [HOGAR_JAVA]`/bin/keytool -export -v -alias ads-credentials`
   ...
   El archivo de certificado denominado &quot;ads-ca.cer&quot; se crea en el directorio [appserverdomain]/adobe/[*nombre del servidor*].
...
1. Inserte el certificado en un nuevo archivo de almacén de claves (el almacén de claves de confianza personalizada) introduciendo el siguiente comando:
   ...
   [HOGAR_JAVA] `/bin/keytool -import -v -noprompt -alias bedrock -file "ads-ca.cer" -keystore "ads-ca.jks" -storepass store_password -keypass key_password`

   >[!NOTE]
   >
   >Reemplace `[JAVA_HOME]` con el directorio donde está instalado el JDK y reemplace `store`*_* `password` y `key`*_* `password` *con sus propias contraseñas.*
...
El archivo del almacén de claves de confianza personalizada llamado &quot;ads-ca.jks&quot; se crea en el directorio [appserverdomain]/adobe/&#39;server&#39;.
...
1. En General, en Configuración, asegúrese de que **Puerto de escucha habilitado** y **Puerto de escucha SSL habilitado** estén seleccionados. Si no está activada, haga lo siguiente:
...
1. En el Centro de cambios, haga clic en **Bloquear y editar** para modificar selecciones y valores.
1. Haga clic en **Cambiar** para obtener la lista de almacén de claves como lista desplegable y seleccione **Identidad personalizada y confianza personalizada**.
1. En Identidad, especifique los siguientes valores:

   **Almacén de claves de identidad personalizado**: *[appserverdomain]*/adobe/*[nombre de servidor]*/ads-credentials.jks, donde *[appserverdomain] *es la ruta real y *[nombre de servidor]* es el nombre del servidor de aplicaciones.

   **Tipo de almacén de claves de identidad personalizada**: JKS

   **Frase de contraseña del almacén de claves de identidad personalizada**: *mypassword* (contraseña del almacén de claves de identidad personalizada)

1. En Confianza, especifique los siguientes valores:

   **Nombre de archivo de almacén de claves de confianza personalizada**: `*[appserverdomain]*/adobe/*'server'*/ads-ca.jks`, donde `*[appserverdomain]*` es la ruta real
...
1. En Identidad, especifique los siguientes valores:
   ...
   **Frase de contraseña**: *micontraseña*
