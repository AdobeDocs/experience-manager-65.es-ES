...
* Se espera que el paquete se instale en un servidor limpio en el que no haya instalados otros paquetes de demostración o versiones de paquetes de demostración más antiguas.
...
* **we-gov-forms.pkg.all-&lt;version>.zip**: *el paquete de demostración completo*.
   ...
   * **we-gov-forms.ui.apps-&lt;version>.zip** *: contiene todos los componentes, bibliotecas de cliente, usuarios de muestra, modelos de flujo de trabajo, etc.*
      ...
      * **we-gov-forms.core-&lt;version>.jar**: *contiene todos los servicios de OSGI, la implementación de pasos de flujos de trabajo personalizados, etc.*
...
* Modelo de datos de AEM Forms y MS Dynamics
...
1. Vaya a *https://&lt;aemserver>:&lt;port>/content/we-gov/home.html?wcmmode=disabled* para garantizar que la instalación se ha realizado correctamente.
...
Para instalar la nueva versión del paquete, siga los pasos descritos en los puntos 4.1 y 4.2. Es posible instalar una versión más reciente del paquete con un paquete anterior instalado, pero se recomienda desinstalar primero la versión anterior. Para ello, siga los pasos que aparecen a continuación.
...
Esta sección contiene detalles sobre la configuración de SSL de la instancia de AEM para poder configurar las opciones de Adobe Sign Cloud.
...
1. Vaya a https://&lt;aemserver>:&lt;port>/aem/inbox, donde podrá completar el proceso explicado en el vínculo de la documentación de referencia anterior.
...
1. Vaya a:
   ...
   *https://&lt;aemserver>:&lt;port>/libs/adobesign/cloudservices/adobesign.html/conf/we-gov*
   ...
   >[!NOTE]
   >
   >La URL utilizada para acceder al servidor de AEM debe coincidir con la URL configurada en el URI de redirección de OAuth de Adobe Sign para evitar problemas de configuración (p. ej. *https://&lt;aemserver>:&lt;port>/mnt/overlay/adobesign/cloudservices/adobesign/properties.html*).
...
1. Introduzca la siguiente información:
   ...
   1. **Configuración de autenticación** (consulte [Configuración en la nube de MS Dynamics](../../forms/using/forms-install-configure-gov-reference-site.md#dynamicsconfig) para recopilar esta información):
      ...
      1. URL de OAuth: p. ej., [https://login.windows.net/common/oauth2/authorize](https://login.windows.net/common/oauth2/authorize).
      1. Actualizar URL del token: p. ej. [https://login.windows.net/common/oauth2/token](https://login.windows.net/common/oauth2/token).
      1. URL del token de acceso: p. ej., [https://login.windows.net/common/oauth2/token](https://login.windows.net/common/oauth2/token).
...
Los pasos detallados en esta sección se incluyen para ayudarle a localizar el ID de cliente, el Secreto del cliente y los datos de la instancia en la nube de MS Dynamics.
...
1. En la vista Configuración, haga clic en &quot;Claves&quot; para ver cómo crear una clave nueva (se utiliza como Secreto de cliente en AEM).
...
1. Los detalles sobre la URL del token de actualización y acceso están disponibles aquí:
   ...
   *[https://learn.microsoft.com/es-es/rest/api/datacatalog/authenticate-a-client-app](https://learn.microsoft.com/es-es/rest/api/datacatalog/authenticate-a-client-app)*
...
1. Seleccione &quot;FDM de Microsoft Dynamics CRM de We.gov &quot; y, luego, &quot;Propiedades&quot;.
...
### Configuración del servicio Adobe Analytics Cloud {#adobe-analytics-cloud-service-configuration}
...
1. Tenga en cuenta que el nombre de usuario incluirá dos puntos en el nombre. Toda la información situada a la izquierda de los dos puntos será el nombre de usuario, y toda la información a la derecha formará parte del nombre de la compañía.
...
1. Tenga en cuenta que, una vez que se han asignado los permisos, la capacidad de un usuario para iniciar sesión puede retrasarse unas horas.
...
>[!NOTE]
Los datos de AEM Forms Analytics están disponibles sin conexión o sin una configuración en la nube de Adobe Analytics si el paquete `we-gov-forms.ui.analytics-<version>.zip` está instalado, pero los datos de AEM Sites requieren una configuración en la nube activa.
...
Para instalar y configurar AEM Forms con formularios de Adobe, los usuarios de la herramienta de conversión deben disponer de los siguientes elementos.
...
Revise los siguientes puntos antes de continuar leyendo las instrucciones:
...
Una vez completada la configuración de IMS, podemos proceder a revisar la configuración en la nube en AEM. Si la configuración no existe, siga estos pasos para crear la configuración en la nube en AEM:

1. Abra el explorador y vaya a la URL del sistema https://&lt;domain_name>:&lt;system_port>.

1. Haga clic en Adobe Experience Manager en la esquina superior izquierda de la pantalla y después en > Herramientas > Cloud Services > Configuración de la conversión de formularios automatizada.
...
1. Haga clic en Forms > Formularios y documentos > Formularios de We.Gov de AEM Forms > AFC.

1. Seleccione el PDF de la solicitud de We.Gov.

1. Haga clic en la opción **Iniciar conversión automatizada** de la esquina superior derecha.
...
1. Una vez seleccionado el botón, se presentarán a los usuarios las siguientes opciones.
   ...
   * Asegúrese de que los usuarios seleccionen la configuración de *Producción de AFC de We.Gov*.
...
1. Seleccione Iniciar conversión una vez que haya configurado todas las opciones que desee utilizar.
...
1. Asegúrese de que la carpeta de plantillas tiene el permiso jcr:leer para todos habilitado, o el usuario del servicio no podrá leer la plantilla del repositorio y se producirá un error durante la conversión.
...
Estos formularios se pueden personalizar para adaptarlos a determinados casos de uso. Tenga en cuenta que algunos campos y la lógica de envío no deben modificarse para garantizar que el formulario sigue funcionando correctamente. Esto incluye:
...
* Enviar: la lógica del botón de envío requiere personalización para admitir devoluciones de llamada. La personalización está documentada, pero se necesitaba un script de gran tamaño para enviar el formulario mientras se realizaba una operación de POST y GET en MS Dynamics a través del modelo de datos de formulario.
...
* (AGRS-120) Actualmente, el componente Navegación de Sites no admite páginas secundarias anidadas con una profundidad de más de 2 niveles.
...
Para realizar las pruebas de accesibilidad, primero debe instalar el complemento de Chrome, que puede encontrar [aquí](https://chrome.google.com/webstore/detail/accessibility-developer-t/fpkknkljclfencbdbgkenhalefipecmb?hl=es).
...
Si los usuarios desean exportar el informe, pueden hacer clic en los tres botones que hay a la derecha de la pantalla y seleccionar una de las opciones adicionales que ofrece el complemento.
...
Si desean configurarlas para que se activen en el flujo de trabajo, los usuarios deben realizar las siguientes tareas.
...
1. Abra la definición del servicio y habilite los servicios seleccionados para que se invoquen en el flujo de trabajo.
   ...
   >[!NOTE]
   Aunque un usuario habilite el servicio desde la página Administrador de configuración, el resto tendrá que establecer una configuración de servicio para comunicarse con los servicios externos solicitados.

   ![Paquete de formularios de we gov](assets/aftia-configuration-options.jpg)

1. Una vez finalizado el proceso, haga clic en el botón Guardar para guardar la configuración.
