...
AEM Con el fin de trabajar mediante programación con etiquetas o ampliar las etiquetas dentro de una aplicación de personalizada, esta página describe el uso de
...
Para la implementación basada en JCR, que asigna `Tags` en JCR `Nodes`, puede utilizar directamente el de Sling `adaptTo` mecanismo si tiene el recurso (por ejemplo, `/content/cq:tags/default/my/tag`):
...
`CQ.tagging.TagInputField` es un widget de formulario para introducir etiquetas. Tiene un menú emergente para seleccionar entre las etiquetas existentes, incluye finalización automática y muchas otras funciones. Su xtype es `tags`.
...
`currentPage` y `slingRequest` están disponibles en un JSP a través del [&lt;cq:definedobjects>](/help/sites-developing/taglib.md) etiqueta.
