...
Para protegerse contra el spam, es posible limitar la frecuencia de los miembros que publican contenido. Además, es posible limitar automáticamente las contribuciones de los miembros recién registrados.
...
Por ejemplo, si el nombre del sitio fuese &quot;*enganchar*&quot; para un sitio llamado &quot;We.Retail Engage&quot;, uno de los grupos de usuarios creados sería:
...
| Administradores de habilitación de comunidades | El grupo Administradores de habilitación de la comunidad está formado por usuarios que están disponibles para su asignación para administrar el grupo Administradores de habilitación de un sitio de la comunidad. |
| Comunidad &lt;*nombre del sitio* > Siteenablementmanagers | El grupo Administradores de habilitación de sitios de la comunidad está formado por usuarios asignados para administrar la habilitación de un sitio de la comunidad [recursos](resources.md). |
...
### Rol de administrador de habilitación de comunidad {#community-enablement-manager-role}

La capacidad de un visitante de un sitio para registrarse automáticamente no suele estar permitida para un [comunidad de habilitación](overview.md#enablement-community) ya que hay costes asociados a cada miembro. Los alumnos y recursos de habilitación son administrados por un usuario asignado al [función](#author-group-roles) de `enablement manager` [durante la creación del sitio](sites-console.md#enablement) en autor (agregado como miembro del grupo) `Community <site-name> Siteenablementmanagers`). El `enablement manager` también es responsable de [asignación de recursos de aprendizaje](resources.md) a los miembros de la comunidad en autor.

Solo los usuarios que son miembros de la global `Community Enablement Managers` grupo puede ser seleccionado como `enablement manager` para un sitio específico de la comunidad.

Para crear un usuario al que se le pueda asignar la función `Community Site Enablement Manager`, utilice la consola de seguridad de la IU clásica para especificar la ruta:

En una instancia de autor:

1. Ha iniciado sesión con privilegios de administrador, vaya a la consola de seguridad de la IU clásica.
...
3. Rellene el `Create User` diálogo.
...
* En el panel izquierdo, busque el usuario recién creado y seleccione para mostrarlo en el panel derecho.
   ...
   ![usuario-comunidad](assets/view-community-user.png)

En el panel izquierdo:

1. Despeje el cuadro de búsqueda y seleccione **[!UICONTROL Ocultar usuarios]**.
2. Localizar y arrastrar `community-enablementmanagers` a la **[!UICONTROL Grupos]** del nuevo usuario mostrado en el panel derecho.
...
Siga los mismos pasos que para crear y asignar un usuario a la función de [administrador de habilitación](#communitysiteenablementmanagerrole), pero añada c `ommunity-administrators` en la pestaña Grupos del usuario.
