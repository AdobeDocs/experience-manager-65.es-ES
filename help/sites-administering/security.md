...
Los grupos son colecciones de usuarios u otros grupos, o ambos. Todas estas colecciones se denominan Miembros de un grupo.
...
AEM WCM instala varios usuarios y grupos. Estas colecciones se ven cuando accede por primera vez a la Consola de seguridad después de la instalación.
...
*Cambiar todas las contraseñas predeterminadas* (si no elimina la cuenta en sí en determinadas circunstancias).
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Usuario</td>
   <td><p>Cuenta de administración del sistema con derechos de acceso completos.</p> <p>Esta cuenta se utiliza para la conexión entre AEM WCM y CRX.</p> <p>Si elimina accidentalmente esta cuenta, se vuelve a crear al reiniciar el repositorio (en la configuración predeterminada).</p> <p>La cuenta de administrador es un requisito de la plataforma AEM. Como consecuencia, esta cuenta no se puede eliminar.</p> </td>
   <td><p>Adobe recomienda cambiar la contraseña predeterminada de esta cuenta de usuario.</p> <p>Preferentemente en la instalación, aunque se puede hacer después.</p> <p>Nota: No confunda esta cuenta con la cuenta de administrador del motor Servlet de CQ.</p> </td>
  </tr>
  <tr>
   <td><p>anónimo</p> <p> </p> </td>
   <td>Usuario</td>
   <td><p>Contiene los derechos predeterminados para el acceso no autenticado a una instancia. De forma predeterminada, esta cuenta tiene los derechos de acceso mínimos.</p> <p>Si elimina accidentalmente esta cuenta, se vuelve a crear al inicio. No se puede eliminar permanentemente, pero se puede deshabilitar.</p> </td>
   <td>Evite eliminar o deshabilitar esta cuenta, ya que afecta negativamente al funcionamiento de las instancias de autor. Si hay requisitos de seguridad que le obliguen a eliminarlo, asegúrese de probar correctamente primero los efectos que tiene en sus sistemas.</td>
  </tr>
  <tr>
   <td><p>autor</p> <p>Contraseña predeterminada: author</p> </td>
   <td>Usuario</td>
   <td><p>Una cuenta de autor con permiso para escribir en /content. Incluye privilegios de colaborador y de internauta.</p> <p>Se puede utilizar como administrador web, ya que tiene acceso a todo el directorio /content.</p> <p>Esta cuenta no es un usuario integrado, sino otro usuario de demostración de Geometrixx</p> </td>
   <td><p>Adobe recomienda que la cuenta se elimine por completo o que se cambie la contraseña predeterminada.</p> <p>Preferentemente en la instalación, aunque se puede hacer después.</p> </td>
  </tr>
  <tr>
   ...
   <td>Incluso si establece un nodo "deny-all" en un nodo, los administradores pueden acceder al nodo</td>
  </tr>
  <tr>
  ...
  <tr>
   <td>colaborador</td>
   <td>Grupo</td>
   <td><p>Privilegios básicos que permiten al usuario escribir contenido (como en, solo funcionalidad).</p> <p>No asigna ningún privilegio al directorio /content. Debe asignarse a grupos o usuarios individuales.</p> </td>
   <td> </td>
  </tr>
  <tr>
   <td>dam-users</td>
  ...
  <tr>
   ...
   <td>Grupo</td>
   <td><p>Un usuario que participa en un flujo de trabajo debe ser miembro de un grupo de usuarios del flujo de trabajo. Concede al usuario acceso completo a: /etc/workflow/instances para que puedan actualizar la instancia del flujo de trabajo.</p> <p>El grupo se incluye en la instalación estándar, pero debe agregar manualmente los usuarios al grupo.</p> </td>
  </tr>
 </tbody>
</table>
...
Cuando la marca de verificación está en la cuadrícula, también indica qué permisos tienen los usuarios en qué ubicaciones dentro de AEM (es decir, qué rutas).
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>El usuario puede:</p>
    ...
    </ul> <p>En el nivel JCR, los usuarios pueden editar un recurso editando sus propiedades, bloqueando, versionando, sin modificaciones, y tienen permisos de escritura completos en los nodos que definen un nodo secundario jcr:content. Por ejemplo, cq:Page, nt:file, cq:Asset.</p> </td>
  </tr>
  <tr>
   <td>Crear</td>
   <td><p>El usuario puede:</p>
    <ul>
     <li>cree una página o página secundaria.</li>
    </ul> <p>If <strong>modificar</strong> se deniega, los subárboles debajo de jcr:content se excluyen porque la creación de jcr:content y sus nodos secundarios se consideran una modificación de página. Esta regla solo se aplica a los nodos que definen un nodo hijo jcr:content.</p> </td>
  </tr>
  <tr>
   <td>Eliminar</td>
   <td><p>El usuario puede:</p>
    ...
    </ul> <p>If <strong>modificar</strong> se deniega cualquier subárbol debajo de jcr:content se excluye al eliminar jcr:content y sus nodos secundarios se consideran una modificación de página. Esta regla solo se aplica a los nodos que definen un nodo hijo jcr:content.</p> </td>
  </tr>
  <tr>
   <td>Leer ACL</td>
...
>[!NOTE]
>
>AEM genera automáticamente grupos de usuarios para la asignación de funciones (propietario, editor, visualizador) en [Colecciones](/help/assets/manage-collections.md). Sin embargo, la adición manual de ACL para estos grupos puede introducir vulnerabilidades de seguridad dentro de AEM. Adobe recomienda que evite añadir ACL manualmente.
...
Las listas de control de acceso están formadas por permisos individuales y se utilizan para determinar el orden en que se aplican estos permisos. La lista se forma según la jerarquía de las páginas consideradas. A continuación, esta lista se analiza de abajo hacia arriba hasta encontrar el primer permiso apropiado para aplicar a una página.

>[!NOTE]
>
>Hay ACL que se incluyen en los ejemplos. Se recomienda que revise y determine qué es lo apropiado para sus aplicaciones. Para revisar las ACL incluidas, vaya a **CRXDE** y seleccione **Control de acceso** para los siguientes nodos:
>
>* `/etc/cloudservices`
...
>
>Al crear ACL específicas para comunidades, los miembros que se unan a esas comunidades pueden recibir permisos adicionales. Por ejemplo, cuando los usuarios se unen a las comunidades en: `/content/we-retail/us/en/community`
...
>
>A diferencia de las versiones anteriores de CQ, **crear** y **delete** no debe concederse más si un usuario solo debe modificar páginas. En su lugar, conceda la variable **modificar** solo si desea que los usuarios puedan crear, modificar o eliminar componentes en páginas existentes.
>
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Enumera las entradas no efectivas que pueden afectar a otro lugar del árbol (tal y como indica un atributo especial presente con el ACE correspondiente que limita el ámbito de la entrada). Como alternativa, es una entrada cuyo efecto se revoca mediante otra entrada definida en la ruta dada o en un nodo antecesor.</td>
  </tr>
 </tbody>
</table>
...
* No asigne permisos directamente a los usuarios. Asignarlos solo a grupos.
   ...
   Al hacerlo, se simplifica el mantenimiento, ya que el número de grupos es mucho menor que el número de usuarios y también es menos volátil.
...
Es decir, una cuenta de usuario puede especificar otras cuentas que pueden operar con su cuenta. Por ejemplo, si el usuario B puede suplantar al usuario A, el usuario B puede actuar utilizando los detalles completos de la cuenta del usuario A.

Esta funcionalidad permite que las cuentas de imitador completen tareas como si estuvieran utilizando la cuenta que están imitando. Por ejemplo, durante una ausencia o para compartir una carga excesiva a corto plazo.
...
>[!CAUTION]
>
>Si una cuenta se hace pasar por otra, es difícil de ver. Se realiza una entrada en el registro de auditoría cuando la suplantación comienza y finaliza, pero los demás archivos de registro (como el registro de acceso) no contienen información de que se haya producido una suplantación en los eventos. Por lo tanto, si el usuario B está suplantando al usuario A, todos los eventos parecen como si el usuario A los hubiera realizado.
...
| Regla | Motivo |
|--- |--- |
| *Usar grupos* | Evite asignar derechos de acceso usuario por usuario. Hay varias razones para este consejo:<ul><li>Tiene muchos más usuarios que grupos, por lo que los grupos simplifican la estructura.</li><li>Los grupos ayudan a proporcionar información general sobre todas las cuentas.</li> <li>La herencia es más sencilla con los grupos.</li><li>Los usuarios van y vienen. Los grupos son a largo plazo.</li></ul> |
...
| Pestaña | Descripción |
|--- |--- |
| Cuadro de filtro | Mecanismo para filtrar a los usuarios, grupos o ambos incluidos en la lista. Consulte [Filtrado de usuarios y grupos](#filtering-users-and-groups). |
| Ocultar usuarios | Un conmutador que oculta a todos los usuarios enumerados y solo deja grupos. Consulte [Ocultar usuarios y grupos](#hiding-users-and-groups). |
| Ocultar grupos | Conmutador que oculta todos los grupos enumerados, dejando solo usuarios. Consulte [Ocultar usuarios y grupos](#hiding-users-and-groups). |
...
1. Haga clic en **OK** para confirmar o Cancelar.
...
>
>Si utiliza AEM Forms en JEE, no utilice las instrucciones siguientes para cambiar la contraseña, sino AEM Forms en el Admin Console JEE (/adminui) para cambiar la contraseña.
...
1. Escriba la nueva contraseña dos veces; como no se muestran en texto sin formato, esta acción se realiza para confirmación; si no coinciden, el sistema muestra un error.
...
1. Vaya a la ruta a la que desea agregar permisos y haga clic en **Detalles**. La parte inferior de la ventana de detalles proporciona información sobre quién tiene permisos para esa página.
...
1. Haga doble clic en el nombre del grupo o cuenta de usuario que desea eliminar de un grupo.
...
* Búsqueda de rutas : si la cadena de búsqueda comienza con &quot;/&quot;, entonces busca los subnodos directos de la ruta dada:
...
| Tecla Enter (Return) | Selecciona un subnodo y lo carga en la cuadrícula del árbol |
...
Puede especificar uno o más usuarios a los que se permite suplantar al usuario actual. Esta capacidad significa que pueden cambiar la configuración de su cuenta a la del usuario actual y actuar en nombre de este usuario.
...
Si planea implementar privilegios específicos de la aplicación, la siguiente información describe lo que debe saber para implementar un privilegio personalizado y cómo aplicarlo en todo CQ:
