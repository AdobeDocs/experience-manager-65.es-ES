...
description: Aprenda a utilizar la variable [!DNL Admin Console] en AEM.
...
AEM 6.4.3.0 introduce [!DNL Admin Console] compatibilidad con instancias de AEM y autenticación basada en Adobe IMS (Identity Management System) para **AEM Managed Services** clientes.

AEM incorporación al [!DNL Admin Console] permitirá a AEM clientes de Managed Services administrar todos los usuarios de Experience Cloud en una consola. Los usuarios se pueden asignar a perfiles de producto asociados con instancias de AEM, lo que les permite iniciar sesión en una instancia específica.
...
* La variable [!DNL Admin Console] representará a AEM clientes de Managed Services como organizaciones de IMS y a sus instancias como contextos de producto. Los administradores de sistemas de clientes y productos podrán administrar el acceso a las instancias
* AEM Managed Services sincronizará las topologías de los clientes con el [!DNL Admin Console]. Habrá una instancia de AEM contexto de producto de Managed Services por instancia en la [!DNL Admin Console].
...
Como primer paso, los clientes deben tener una organización aprovisionada en Adobe IMS. Los clientes de Adobe Enterprise están representados como organizaciones de IMS en la variable [Adobe [!DNL Admin Console]](https://helpx.adobe.com/es/enterprise/using/admin-console.html).

AEM los clientes de Managed Services ya deben disponer de una organización aprovisionada y, como parte del aprovisionamiento de IMS, las instancias de cliente estarán disponibles en el [!DNL Admin Console] para administrar las autorizaciones de usuario y el acceso.
...
1. El administrador del sistema configura el proveedor de identidad (IDP) en la variable [!DNL Admin Console] para la configuración de SSO.
...
### Incorporación de usuarios al [!DNL Admin Console] {#onboarding-users-to-the-admin-console}
...
#### Adición manual hasta [!DNL Admin Console] IU {#manual-addition-through-admin-console-ui}

Los usuarios y grupos se pueden crear manualmente en la [!DNL Admin Console] IU. Este método se puede utilizar si no tienen un gran número de usuarios que administrar. Por ejemplo, un número menor de 50 usuarios AEM.
...
#### Carga de archivos en el [!DNL Admin Console] IU {#file-upload-in-the-admin-console-ui}
...
Cuando se ejecuta la sincronización de usuarios, se obtiene una lista de usuarios de Active Directory de la organización (o de cualquier otra fuente de datos compatible) y se compara con la lista de usuarios de la [!DNL Admin Console]. A continuación, llama al Adobe [!DNL User Management] para que la [!DNL Admin Console] se sincroniza con el directorio de la organización. El flujo de cambios es totalmente unidireccional; cualquier edición realizada en la variable [!DNL Admin Console] no se inserte en el directorio .

La herramienta permite que el administrador del sistema asigne grupos de usuarios en el directorio del cliente con la configuración del producto y los grupos de usuarios en el [!DNL Admin Console], la nueva versión del UST también permite la creación dinámica de grupos de usuarios en la variable [!DNL Admin Console].

Para configurar la sincronización de usuarios, la organización debe crear un conjunto de credenciales de la misma manera que usaría la variable [[!DNL User Management] API](https://www.adobe.io/apis/cloudplatform/usermanagement/docs/setup.html).
...
Las principales características de esta versión son la capacidad de asignar dinámicamente nuevos grupos LDAP para la pertenencia de los usuarios a la [!DNL Admin Console], así como la creación dinámica de grupos de usuarios.
...
Cuando el administrador de productos del cliente inicia sesión en [!DNL Admin Console], verán varias instancias del contexto de producto de Managed Services de AEM como se muestra a continuación:
...
Cualquier usuario agregado bajo este perfil de producto podrá iniciar sesión en esa instancia, como se muestra en el ejemplo siguiente:
...
Si se configura un IDP federado durante la [!DNL Admin Console] configuración, el usuario será redirigido al IDP del cliente para SSO.
