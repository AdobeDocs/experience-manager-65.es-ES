...
# AEM Autenticación IMS de Adobe y compatibilidad con [!DNL Admin Console] para Managed Services de la {#adobe-ims-authentication-and-admin-console-support-for-aem-managed-services}
...
AEM AEM La versión 6.4.3.0 incorpora compatibilidad con [!DNL Admin Console] para instancias de y autenticación basada en Adobe IMS (Identity Management AEM System) para clientes de **Managed Services**.

AEM AEM La incorporación a [!DNL Admin Console] permitirá a los clientes de Managed Services de la administrar todos los usuarios de Experience Cloud en una consola. AEM Los usuarios pueden asignarse a perfiles de producto asociados a instancias de, lo que les permite iniciar sesión en una instancia específica.
...
* AEM [!DNL Admin Console] representará a los clientes de Managed Services como organizaciones de IMS y sus instancias como contextos de producto. Los administradores de sistemas de clientes y productos podrán administrar el acceso a las instancias
* AEM Managed Services sincronizará las topologías de clientes con [!DNL Admin Console]. AEM Habrá una instancia de contexto de producto de Managed Services por instancia en el [!DNL Admin Console].
...
* [!DNL User Management] (en el Adobe [!DNL Admin Console]) seguirá siendo propiedad de los administradores del cliente.
...
AEM La incorporación del cliente a [!DNL Admin Console] es un requisito previo para usar Adobe IMS para la autenticación de la.

Como primer paso, los clientes deben disponer de una organización en Adobe IMS. Los clientes de Adobe Enterprise están representados como organizaciones de IMS en el [Adobe [!DNL Admin Console]](https://helpx.adobe.com/es/enterprise/using/admin-console.html).

AEM Los clientes de Managed Services ya deben tener una organización aprovisionada y, como parte del aprovisionamiento de IMS, las instancias de cliente estarán disponibles en [!DNL Admin Console] para administrar los derechos de usuario y el acceso.
...
>[!NOTE]
>
>Para obtener más información acerca de los conceptos básicos de Identity Management de Adobe, incluida la configuración de IDP, consulte el artículo [esta página.](https://helpx.adobe.com/es/enterprise/using/set-up-identity.html)
>
>Para obtener más información acerca de Enterprise Administration y [!DNL Admin Console], consulte el artículo [esta página](https://helpx.adobe.com/es/enterprise/managing/user-guide.html).
...
Los usuarios y grupos se pueden crear manualmente en la interfaz de usuario de [!DNL Admin Console]. Este método se puede utilizar si no tienen muchos usuarios que administrar. AEM Por ejemplo, menos de 50 usuarios de la.
...
>
>La documentación de Adobe Developer Console se encuentra [aquí](https://developer.adobe.com/developer-console/docs/guides/).
>
>
>La API [!DNL User Management] que usa la herramienta de sincronización de usuarios se cubre en esta [ubicación](https://adobe-apiplatform.github.io/umapi-documentation/en/).
...
AEM Cuando el administrador de productos del cliente inicia sesión en [!DNL Admin Console], puede ver varias instancias del contexto de producto de Managed Services de la manera más frecuente, tal y como se muestra a continuación:
