...
Por ejemplo, cree una raíz de idioma en `/content/geometrixx` con nombre como `fr_LU` (y el título como francés (Luxemburgo)). Posteriormente, cree una copia de idioma de la página desde el panel de referencias y vaya a `Create structure only` en `Create & Translate`. Finalmente, cree un proyecto de traducción y, a continuación, añada la copia de idioma al trabajo de traducción.
...
Al crear una copia de idioma mediante la IU táctil (**Referencias** -> **Actualizar copia de idioma**), se crea una nueva carpeta DAM en el nuevo idioma y se hace referencia a los recursos desde allí.
...
Para obtener más información detallada, consulte [Guía de migración a CoralUI 3](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/legacy/coral2/migration.html).
